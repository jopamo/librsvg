...
enum CRStatus cr_rgb_set(CRRgb* a_this, gulong a_red, gulong a_green, gulong a_blue, gboolean a_is_percentage) {
    ...
    if (a_is_percentage != FALSE) {
        g_return_val_if_fail(a_red <= 100 && a_green <= 100 && a_blue <= 100, CR_BAD_PARAM_ERROR);
    }
...
static int cr_rgb_color_name_compare(const void* a, const void* b) {
...
enum CRStatus cr_rgb_set_from_name(CRRgb* a_this, const guchar* a_color_name) {
    ...
    result = bsearch(a_color_name, gv_standard_colors, G_N_ELEMENTS(gv_standard_colors), sizeof(gv_standard_colors[0]),
...
enum CRStatus cr_rgb_set_from_hex_str(CRRgb* a_this, const guchar* a_hex) {
    ...
    if (status == CR_OK) {
        status = cr_rgb_set(a_this, colors[0], colors[1], colors[2], FALSE);
...
enum CRStatus cr_rgb_set_from_term(CRRgb* a_this, const struct _CRTerm* a_value) {
    enum CRStatus status = CR_OK;
    g_return_val_if_fail(a_this && a_value, CR_BAD_PARAM_ERROR);

    switch (a_value->type) {
        case TERM_RGB:
            if (a_value->content.rgb) {
                cr_rgb_set_from_rgb(a_this, a_value->content.rgb);
            }
            break;
        case TERM_IDENT:
            if (a_value->content.str && a_value->content.str->stryng && a_value->content.str->stryng->str) {
                if (!strncmp("inherit", a_value->content.str->stryng->str, sizeof("inherit") - 1)) {
                    ...
                    status = cr_rgb_set_from_name(a_this, (const guchar*)a_value->content.str->stryng->str);
                }
            }
            else {
                cr_utils_trace_info("a_value has NULL string value");
            }
            break;
        case TERM_HASH:
            if (a_value->content.str && a_value->content.str->stryng && a_value->content.str->stryng->str) {
                status = cr_rgb_set_from_hex_str(a_this, (const guchar*)a_value->content.str->stryng->str);
            }
            else {
                cr_utils_trace_info("a_value has NULL string value");
            }
