...
CRPropList* cr_prop_list_append(CRPropList* a_this, CRPropList* a_to_append) {
    ...
    for (cur = a_this; cur && PRIVATE(cur) && PRIVATE(cur)->next; cur = PRIVATE(cur)->next)
        ;
...
CRPropList* cr_prop_list_prepend(CRPropList* a_this, CRPropList* a_to_prepend) {
    ...
    for (cur = a_to_prepend; cur && PRIVATE(cur)->next; cur = PRIVATE(cur)->next)
        ;
    g_return_val_if_fail(cur, NULL);
...
CRPropList* cr_prop_list_prepend2(CRPropList* a_this, CRString* a_prop_name, CRDeclaration* a_decl) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_prop_name && a_decl, NULL);
...
enum CRStatus cr_prop_list_set_decl(CRPropList* a_this, CRDeclaration* a_decl) {
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_decl, CR_BAD_PARAM_ERROR);
...
enum CRStatus cr_prop_list_lookup_prop(CRPropList* a_this, CRString* a_prop, CRPropList** a_pair) {
    ...
    for (cur = a_this; cur; cur = PRIVATE(cur)->next) {
        if (PRIVATE(cur)->prop && PRIVATE(cur)->prop->stryng && PRIVATE(cur)->prop->stryng->str && a_prop->stryng &&
            a_prop->stryng->str && !strcmp(PRIVATE(cur)->prop->stryng->str, a_prop->stryng->str))
            break;
    }
...
void cr_prop_list_destroy(CRPropList* a_this) {
    ...
    for (tail = a_this; tail && PRIVATE(tail) && PRIVATE(tail)->next; tail = cr_prop_list_get_next(tail))
        ;
