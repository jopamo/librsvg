...
#define CHECK_PARSING_STATUS_ERR(a_this, a_status, a_is_exception, a_err_msg, a_err_status) \
    if ((a_status) != CR_OK) {                                                              \
        if (a_is_exception == FALSE)                                                        \
            a_status = CR_PARSING_ERROR;                                                    \
...
#define RECORD_INITIAL_POS(a_this, a_pos)                         \
    status = cr_tknzr_get_cur_pos(PRIVATE(a_this)->tknzr, a_pos); \
...
#define RECORD_CUR_BYTE_ADDR(a_this, a_addr)                             \
    status = cr_tknzr_get_cur_byte_addr(PRIVATE(a_this)->tknzr, a_addr); \
    CHECK_PARSING_STATUS(status, TRUE)
...
#define PEEK_BYTE(a_parser, a_offset, a_byte_ptr)                              \
    status = cr_tknzr_peek_byte(PRIVATE(a_this)->tknzr, a_offset, a_byte_ptr); \
    CHECK_PARSING_STATUS(status, TRUE);
...
#define SKIP_BYTES(a_this, a_nb_bytes)                                             \
    status = cr_tknzr_seek_index(PRIVATE(a_this)->tknzr, CR_SEEK_CUR, a_nb_bytes); \
    CHECK_PARSING_STATUS(status, TRUE);
...
#define SKIP_CHARS(a_parser, a_nb_chars)                                         \
    {                                                                            \
        ...
        status = cr_tknzr_consume_chars(PRIVATE(a_parser)->tknzr, 0, &nb_chars); \
        CHECK_PARSING_STATUS(status, TRUE);                                      \
    }
...
#define GET_NEXT_TOKEN(a_this, a_token_ptr)                                \
    status = cr_tknzr_get_next_token(PRIVATE(a_this)->tknzr, a_token_ptr); \
    ENSURE_PARSING_COND(status == CR_OK);

#ifdef WITH_UNICODE_ESCAPE_AND_RANGE
static enum CRStatus cr_parser_parse_unicode_escape(CRParser* a_this, guint32* a_unicode);
static enum CRStatus cr_parser_parse_escape(CRParser* a_this, guint32* a_esc_code);

static enum CRStatus cr_parser_parse_unicode_range(CRParser* a_this, CRString** a_inf, CRString** a_sup);
#endif
...
static CRParserError* cr_parser_error_new(const guchar* a_msg, enum CRStatus);
 ...
 */
static CRParserError* cr_parser_error_new(const guchar* a_msg, enum CRStatus a_status) {
...
static enum CRStatus cr_parser_push_error(CRParser* a_this, const guchar* a_msg, enum CRStatus a_status) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_msg, CR_BAD_PARAM_ERROR);
    ...
    cr_parser_error_set_pos(error, pos.line, pos.col, pos.next_byte_index - 1);

    PRIVATE(a_this)->err_stack = g_list_prepend(PRIVATE(a_this)->err_stack, error);
...
static enum CRStatus cr_parser_clear_errors(CRParser* a_this) {
    ...
    for (cur = PRIVATE(a_this)->err_stack; cur; cur = cur->next) {
        if (cur->data) {
            cr_parser_error_destroy((CRParserError*)cur->data);
        }
    }
...
enum CRStatus cr_parser_try_to_skip_spaces_and_comments(CRParser* a_this) {
    ...
    do {
        ...
        if (status != CR_OK)
            goto error;
    } while ((token != NULL) && (token->type == COMMENT_TK || token->type == S_TK));
...
error:
    cr_parser_push_error(a_this, (const guchar*)"could not recognize next production", CR_ERROR);
...
static enum CRStatus cr_parser_parse_atrule_core(CRParser* a_this) {
    ...
    ENSURE_PARSING_COND(status == CR_OK && token &&
                        (token->type == ATKEYWORD_TK || token->type == IMPORT_SYM_TK || token->type == PAGE_SYM_TK ||
                         token->type == MEDIA_SYM_TK || token->type == FONT_FACE_SYM_TK ||
...
static enum CRStatus cr_parser_parse_ruleset_core(CRParser* a_this) {
    ...
    ENSURE_PARSING_COND(status == CR_OK || status == CR_PARSING_ERROR || status == CR_END_OF_INPUT_ERROR);

    status = cr_tknzr_get_next_token(PRIVATE(a_this)->tknzr, &token);
    ENSURE_PARSING_COND(status == CR_OK && token && token->type == CBO_TK);
    cr_token_destroy(token);
    token = NULL;
...
parse_declaration_list:
    ...
    ENSURE_PARSING_COND(status == CR_OK && token && token->type == SEMICOLON_TK);
...
static enum CRStatus cr_parser_parse_declaration_core(CRParser* a_this) {
    ...
    status = cr_tknzr_get_next_token(PRIVATE(a_this)->tknzr, &token);
    ENSURE_PARSING_COND(status == CR_OK && token && token->type == DELIM_TK && token->u.unichar == ':');
    cr_token_destroy(token);
    token = NULL;
    cr_parser_try_to_skip_spaces_and_comments(a_this);
...
static enum CRStatus cr_parser_parse_any_core(CRParser* a_this) {
    ...
    switch (token1->type) {
        ...
        case FUNCTION_TK:
            ...
            status = cr_tknzr_get_next_token(PRIVATE(a_this)->tknzr, &token2);
            ENSURE_PARSING_COND(status == CR_OK && token2 && token2->type == PC_TK);
            break;
        case PO_TK:
            status = cr_tknzr_get_next_token(PRIVATE(a_this)->tknzr, &token2);
            ...
            if (token2->type == PC_TK) {
                ...
                status = cr_tknzr_unget_token(PRIVATE(a_this)->tknzr, token2);
                token2 = NULL;
            }
        ...
        case BO_TK:
            ...
            status = cr_tknzr_get_next_token(PRIVATE(a_this)->tknzr, &token2);
            ENSURE_PARSING_COND(status == CR_OK && token2 && token2->type == BC_TK);
            status = CR_OK;
            break;
        default:
            status = CR_PARSING_ERROR;
...
static enum CRStatus cr_parser_parse_attribute_selector(CRParser* a_this, CRAttrSel** a_sel) {
    ...
    status = cr_tknzr_get_next_token(PRIVATE(a_this)->tknzr, &token);
    ENSURE_PARSING_COND(status == CR_OK && token && token->type == BO_TK);
    ...
    cr_parsing_location_copy(&result->location, &location);
    status = cr_tknzr_get_next_token(PRIVATE(a_this)->tknzr, &token);
    ENSURE_PARSING_COND(status == CR_OK && token && token->type == IDENT_TK);
...
parse_right_part:
    ...
    ENSURE_PARSING_COND(status == CR_OK && token && token->type == BC_TK);
done:
    if (token) {
        cr_token_destroy(token);
        token = NULL;
...
static enum CRStatus cr_parser_parse_property(CRParser* a_this, CRString** a_property) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && PRIVATE(a_this)->tknzr && a_property, CR_BAD_PARAM_ERROR);
...
enum CRStatus cr_parser_parse_term(CRParser* a_this, CRTerm** a_term) {
    ...
    if (token->type == EMS_TK || token->type == EXS_TK || token->type == LENGTH_TK || token->type == ANGLE_TK ||
        token->type == TIME_TK || token->type == FREQ_TK || token->type == PERCENTAGE_TK || token->type == NUMBER_TK) {
        ...
        status = cr_tknzr_unget_token(PRIVATE(a_this)->tknzr, token);
        token = NULL;
        status = cr_parser_parse_function(a_this, &func_name, &param);

        if (status == CR_OK) {
            status = cr_term_set_function(result, func_name, param);
            CHECK_PARSING_STATUS(status, TRUE);
        }
    }
...
static enum CRStatus cr_parser_parse_simple_selector(CRParser* a_this, CRSimpleSel** a_sel) {
    ...
    cr_parsing_location_copy(&sel->location, &token->location);
    ...
    for (;;) {
        ...
        if (token && token->type == HASH_TK) {
            ...
            cr_parsing_location_copy(&add_sel->location, &token->location);
            ...
            if (token && token->type == IDENT_TK) {
                ...
                add_sel = cr_additional_sel_new_with_type(CLASS_ADD_SELECTOR);
            ...
            status = cr_parser_parse_attribute_selector(a_this, &attr_sel);
            CHECK_PARSING_STATUS(status, FALSE);

            add_sel = cr_additional_sel_new_with_type(ATTRIBUTE_ADD_SELECTOR);
            ...
            cr_parsing_location_copy(&pseudo->location, &token->location);
            ...
            if (token->type == IDENT_TK) {
                ...
                token->u.str = NULL;
                cr_parser_try_to_skip_spaces_and_comments(a_this);
                status = cr_parser_parse_ident(a_this, &pseudo->extra);
            ...
            if (status == CR_OK) {
                ...
                add_sel = cr_additional_sel_new_with_type(PSEUDO_CLASS_ADD_SELECTOR);
...
static enum CRStatus cr_parser_parse_simple_sels(CRParser* a_this, CRSimpleSel** a_sel) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_sel, CR_BAD_PARAM_ERROR);
...
static enum CRStatus cr_parser_parse_selector(CRParser* a_this, CRSelector** a_selector) {
    ...
    if (simple_sels) {
        selector = cr_selector_append_simple_sel(selector, simple_sels);
        if (selector) {
            cr_parsing_location_copy(&selector->location, &simple_sels->location);
        }
        simple_sels = NULL;
    }
    else {
...
static enum CRStatus cr_parser_parse_function(CRParser* a_this, CRString** a_func_name, CRTerm** a_expr) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_func_name, CR_BAD_PARAM_ERROR);
...
static enum CRStatus cr_parser_parse_uri(CRParser* a_this, CRString** a_str) {
    ...
    status = cr_tknzr_parse_token(PRIVATE(a_this)->tknzr, URI_TK, NO_ET, a_str, NULL);
    return status;
}
...
static enum CRStatus cr_parser_parse_string(CRParser* a_this, CRString** a_str) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && PRIVATE(a_this)->tknzr && a_str, CR_BAD_PARAM_ERROR);

    status = cr_tknzr_parse_token(PRIVATE(a_this)->tknzr, STRING_TK, NO_ET, a_str, NULL);
    return status;
}
...
static enum CRStatus cr_parser_parse_ident(CRParser* a_this, CRString** a_str) {
    ...
    status = cr_tknzr_parse_token(PRIVATE(a_this)->tknzr, IDENT_TK, NO_ET, a_str, NULL);
    return status;
}
...
static enum CRStatus cr_parser_parse_stylesheet(CRParser* a_this) {
    ...
    if (PRIVATE(a_this)->sac_handler && PRIVATE(a_this)->sac_handler->start_document) {
        PRIVATE(a_this)->sac_handler->start_document(PRIVATE(a_this)->sac_handler);
    }
...
parse_charset:
    ...
    if (token && token->type == CHARSET_SYM_TK) {
        ...
        status = cr_parser_parse_charset(a_this, &charset, &location);

        if (status == CR_OK && charset) {
            if (PRIVATE(a_this)->sac_handler && PRIVATE(a_this)->sac_handler->charset) {
                PRIVATE(a_this)->sac_handler->charset(PRIVATE(a_this)->sac_handler, charset, &location);
            }
        }
        else if (status != CR_END_OF_INPUT_ERROR) {
    ...
    else if (token && (token->type == S_TK || token->type == COMMENT_TK)) {
        status = cr_tknzr_unget_token(PRIVATE(a_this)->tknzr, token);
        token = NULL;
        CHECK_PARSING_STATUS(status, TRUE);
    ...
    do {
        ...
        if (status == CR_END_OF_INPUT_ERROR)
            goto done;
        CHECK_PARSING_STATUS(status, TRUE);
    } while (token && (token->type == S_TK || token->type == CDO_TK || token->type == CDC_TK));
    ...
    for (;;) {
        ...
        if (token && token->type == IMPORT_SYM_TK) {
            ...
            status = cr_parser_parse_import(a_this, &media_list, &import_string, &location);
            if (status == CR_OK) {
                if (import_string && PRIVATE(a_this)->sac_handler && PRIVATE(a_this)->sac_handler->import_style) {
                    ...
                    if ((PRIVATE(a_this)->sac_handler->import_style_result)) {
                        PRIVATE(a_this)->sac_handler->import_style_result(PRIVATE(a_this)->sac_handler, media_list,
                                                                          import_string, NULL, NULL);
                    }
                }
            }
            else if (status != CR_END_OF_INPUT_ERROR) {
                if (PRIVATE(a_this)->sac_handler && PRIVATE(a_this)->sac_handler->error) {
                    PRIVATE(a_this)->sac_handler->error(PRIVATE(a_this)->sac_handler);
                }
                status = cr_parser_parse_atrule_core(a_this);
                CHECK_PARSING_STATUS(status, TRUE);
            }
        ...
        else if (token && (token->type == S_TK || token->type == CDO_TK || token->type == CDC_TK)) {
            status = cr_tknzr_unget_token(PRIVATE(a_this)->tknzr, token);
            token = NULL;
...
parse_ruleset_and_others:
    ...
    for (;;) {
        ...
        if (token && (token->type == S_TK || token->type == CDO_TK || token->type == CDC_TK)) {
            status = cr_tknzr_unget_token(PRIVATE(a_this)->tknzr, token);
            token = NULL;
            ...
            do {
                ...
                cr_parser_try_to_skip_spaces_and_comments(a_this);
                status = cr_tknzr_get_next_token(PRIVATE(a_this)->tknzr, &token);
            } while (token && (token->type == S_TK || token->type == COMMENT_TK || token->type == CDO_TK ||
                               token->type == CDC_TK));
            if (token) {
                cr_tknzr_unget_token(PRIVATE(a_this)->tknzr, token);
                token = NULL;
        ...
        else if (token && (token->type == HASH_TK || (token->type == DELIM_TK && token->u.unichar == '.') ||
                           (token->type == DELIM_TK && token->u.unichar == ':') ||
                           (token->type == DELIM_TK && token->u.unichar == '*') || (token->type == BO_TK) ||
                           token->type == IDENT_TK)) {
            /*
    ...
    if (status == CR_END_OF_INPUT_ERROR || status == CR_OK) {
        if (PRIVATE(a_this)->sac_handler && PRIVATE(a_this)->sac_handler->end_document) {
            PRIVATE(a_this)->sac_handler->end_document(PRIVATE(a_this)->sac_handler);
        }
    ...
    if (PRIVATE(a_this)->sac_handler && PRIVATE(a_this)->sac_handler->unrecoverable_error) {
        PRIVATE(a_this)->sac_handler->unrecoverable_error(PRIVATE(a_this)->sac_handler);
    }
...
enum CRStatus cr_parser_parse_file(CRParser* a_this, const guchar* a_file_uri, enum CREncoding a_enc) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_file_uri, CR_BAD_PARAM_ERROR);
...
enum CRStatus cr_parser_parse_expr(CRParser* a_this, CRTerm** a_expr) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_expr, CR_BAD_PARAM_ERROR);
    ...
    for (;;) {
        ...
        status = cr_tknzr_peek_byte(PRIVATE(a_this)->tknzr, 1, &next_byte);
        if (status != CR_OK) {
            if (status == CR_END_OF_INPUT_ERROR) {
                status = CR_OK;
                break;
...
enum CRStatus cr_parser_parse_prio(CRParser* a_this, CRString** a_prio) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_prio && *a_prio == NULL, CR_BAD_PARAM_ERROR);
    ...
    ENSURE_PARSING_COND(status == CR_OK && token && token->type == IMPORTANT_SYM_TK);
...
enum CRStatus cr_parser_parse_declaration(CRParser* a_this,
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_property && a_expr && a_important, CR_BAD_PARAM_ERROR);
    ...
    CHECK_PARSING_STATUS_ERR(a_this, status, FALSE,
                             (const guchar*)"while parsing declaration: next property is malformed", CR_SYNTAX_ERROR);

    READ_NEXT_CHAR(a_this, &cur_char);

    if (cur_char != ':') {
        status = CR_PARSING_ERROR;
        cr_parser_push_error(a_this, (const guchar*)"while parsing declaration: this char must be ':'",
                             CR_SYNTAX_ERROR);
        goto error;
    }
    ...
    CHECK_PARSING_STATUS_ERR(a_this, status, FALSE,
                             (const guchar*)"while parsing declaration: next expression is malformed", CR_SYNTAX_ERROR);
...
enum CRStatus cr_parser_parse_ruleset(CRParser* a_this) {
    ...
    ENSURE_PARSING_COND_ERR(a_this, cur_char == '{', (const guchar*)"while parsing rulset: current char should be '{'",
                            CR_SYNTAX_ERROR);

    if (PRIVATE(a_this)->sac_handler && PRIVATE(a_this)->sac_handler->start_selector) {
        /*
        ...
        PRIVATE(a_this)->sac_handler->start_selector(PRIVATE(a_this)->sac_handler, selector);
        start_selector = TRUE;
    }
    ...
    status = cr_parser_parse_declaration(a_this, &property, &expr, &is_important);
    if (expr) {
        cr_term_ref(expr);
    }
    if (status == CR_OK && PRIVATE(a_this)->sac_handler && PRIVATE(a_this)->sac_handler->property) {
        PRIVATE(a_this)->sac_handler->property(PRIVATE(a_this)->sac_handler, property, expr, is_important);
    }
    if (status == CR_OK) {
        /*
         *free the allocated
...
end_of_ruleset:
    ...
    READ_NEXT_CHAR(a_this, &cur_char);
    ENSURE_PARSING_COND_ERR(a_this, cur_char == '}', (const guchar*)"while parsing rulset: current char must be a '}'",
                            CR_SYNTAX_ERROR);

    selector->location = end_parsing_location;
    if (PRIVATE(a_this)->sac_handler && PRIVATE(a_this)->sac_handler->end_selector) {
        PRIVATE(a_this)->sac_handler->end_selector(PRIVATE(a_this)->sac_handler, selector);
...
error:
    if (start_selector == TRUE && PRIVATE(a_this)->sac_handler && PRIVATE(a_this)->sac_handler->error) {
        PRIVATE(a_this)->sac_handler->error(PRIVATE(a_this)->sac_handler);
    }
    if (expr) {
        cr_term_unref(expr);
...
enum CRStatus cr_parser_parse_import(CRParser* a_this,
    ...
    g_return_val_if_fail(a_this && a_import_string && (*a_import_string == NULL), CR_BAD_PARAM_ERROR);

    RECORD_INITIAL_POS(a_this, &init_pos);

    if (BYTE(a_this, 1, NULL) == '@' && BYTE(a_this, 2, NULL) == 'i' && BYTE(a_this, 3, NULL) == 'm' &&
        BYTE(a_this, 4, NULL) == 'p' && BYTE(a_this, 5, NULL) == 'o' && BYTE(a_this, 6, NULL) == 'r' &&
        BYTE(a_this, 7, NULL) == 't') {
        SKIP_CHARS(a_this, 1);
        if (a_location) {
            cr_parser_get_parsing_location(a_this, a_location);
        }
        SKIP_CHARS(a_this, 6);
        status = CR_OK;
    }
    ...
    for (; status == CR_OK;) {
        if ((status = cr_tknzr_peek_char(PRIVATE(a_this)->tknzr, &next_char)) != CR_OK) {
            if (status == CR_END_OF_INPUT_ERROR) {
                status = CR_OK;
                goto okay;
            }
...
enum CRStatus cr_parser_parse_media(CRParser* a_this) {
    ...
    status = cr_tknzr_get_next_token(PRIVATE(a_this)->tknzr, &token);
    ENSURE_PARSING_COND(status == CR_OK && token && token->type == MEDIA_SYM_TK);
    cr_parsing_location_copy(&location, &token->location);
    cr_token_destroy(token);
    token = NULL;
     ...
     */
    if (PRIVATE(a_this)->sac_handler && PRIVATE(a_this)->sac_handler->start_media) {
        PRIVATE(a_this)->sac_handler->start_media(PRIVATE(a_this)->sac_handler, media_list, &location);
    }
     ...
     */
    if (PRIVATE(a_this)->sac_handler && PRIVATE(a_this)->sac_handler->end_media) {
        PRIVATE(a_this)->sac_handler->end_media(PRIVATE(a_this)->sac_handler, media_list);
    }
...
enum CRStatus cr_parser_parse_page(CRParser* a_this) {
    ...
    CRString *page_selector = NULL, *page_pseudo_class = NULL, *property = NULL;
    ...
    status = cr_tknzr_get_next_token(PRIVATE(a_this)->tknzr, &token);
    ENSURE_PARSING_COND(status == CR_OK && token && token->type == PAGE_SYM_TK);
     ...
     */
    if (PRIVATE(a_this)->sac_handler && PRIVATE(a_this)->sac_handler->start_page) {
        PRIVATE(a_this)->sac_handler->start_page(PRIVATE(a_this)->sac_handler, page_selector, page_pseudo_class,
                                                 &location);
    }
    cr_parser_try_to_skip_spaces_and_comments(a_this);
    ...
    status = cr_parser_parse_declaration(a_this, &property, &css_expression, &important);
    ENSURE_PARSING_COND(status == CR_OK);

    /*
     *call the relevant SAC handler here...
     */
    if (PRIVATE(a_this)->sac_handler && PRIVATE(a_this)->sac_handler->property) {
        if (css_expression)
            cr_term_ref(css_expression);

        PRIVATE(a_this)->sac_handler->property(PRIVATE(a_this)->sac_handler, property, css_expression, important);
    }
    /*
    ...
    status = cr_tknzr_get_next_token(PRIVATE(a_this)->tknzr, &token);
    ENSURE_PARSING_COND(status == CR_OK && token && token->type == CBC_TK);
    cr_token_destroy(token);
    token = NULL;
    /*
     *call the relevant SAC handler here.
     */
    if (PRIVATE(a_this)->sac_handler && PRIVATE(a_this)->sac_handler->end_page) {
        PRIVATE(a_this)->sac_handler->end_page(PRIVATE(a_this)->sac_handler, page_selector, page_pseudo_class);
    }
...
enum CRStatus cr_parser_parse_charset(CRParser* a_this, CRString** a_value, CRParsingLocation* a_charset_sym_location) {
    ...
    g_return_val_if_fail(a_this && a_value && (*a_value == NULL), CR_BAD_PARAM_ERROR);

    RECORD_INITIAL_POS(a_this, &init_pos);

    status = cr_tknzr_get_next_token(PRIVATE(a_this)->tknzr, &token);

    ENSURE_PARSING_COND(status == CR_OK && token && token->type == CHARSET_SYM_TK);
    if (a_charset_sym_location) {
        cr_parsing_location_copy(a_charset_sym_location, &token->location);
    }
    cr_token_destroy(token);
    token = NULL;
    ...
    status = cr_tknzr_get_next_token(PRIVATE(a_this)->tknzr, &token);
    ENSURE_PARSING_COND(status == CR_OK && token && token->type == STRING_TK);
...
enum CRStatus cr_parser_parse_font_face(CRParser* a_this) {
    ...
    status = cr_tknzr_get_next_token(PRIVATE(a_this)->tknzr, &token);
    ENSURE_PARSING_COND(status == CR_OK && token && token->type == FONT_FACE_SYM_TK);
     ...
     */
    if (PRIVATE(a_this)->sac_handler && PRIVATE(a_this)->sac_handler->start_font_face) {
        PRIVATE(a_this)->sac_handler->start_font_face(PRIVATE(a_this)->sac_handler, &location);
    }
    ...
    if (PRIVATE(a_this)->sac_handler->end_font_face) {
        PRIVATE(a_this)->sac_handler->end_font_face(PRIVATE(a_this)->sac_handler);
    }
    cr_parser_try_to_skip_spaces_and_comments(a_this);
...
enum CRStatus cr_parser_get_tknzr(CRParser* a_this, CRTknzr** a_tknzr) {
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_tknzr, CR_BAD_PARAM_ERROR);
...
enum CRStatus cr_parser_get_parsing_location(CRParser const* a_this, CRParsingLocation* a_loc) {
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_loc, CR_BAD_PARAM_ERROR);

    return cr_tknzr_get_parsing_location(PRIVATE(a_this)->tknzr, a_loc);
}
...
enum CRStatus cr_parser_parse_buf(CRParser* a_this, const guchar* a_buf, gulong a_len, enum CREncoding a_enc) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_buf, CR_BAD_PARAM_ERROR);
