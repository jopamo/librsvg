...
static gboolean class_add_sel_matches_node(CRAdditionalSel* a_add_sel, xmlNode* a_node);

static gboolean id_add_sel_matches_node(CRAdditionalSel* a_add_sel, xmlNode* a_node);

static gboolean attr_add_sel_matches_node(CRAdditionalSel* a_add_sel, xmlNode* a_node);
...
static gboolean pseudo_class_add_sel_matches_node(CRSelEng* a_this, CRAdditionalSel* a_add_sel, xmlNode* a_node);

static gboolean lang_pseudo_class_handler(CRSelEng* a_this, CRAdditionalSel* a_sel, xmlNode* a_node);

static gboolean first_child_pseudo_class_handler(CRSelEng* a_this, CRAdditionalSel* a_sel, xmlNode* a_node);
...
#define strqcmp(str, lit, lit_len) (strlen(str) != (lit_len) || memcmp(str, lit, lit_len))
...
static gboolean lang_pseudo_class_handler(CRSelEng* a_this, CRAdditionalSel* a_sel, xmlNode* a_node) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_sel && a_sel->content.pseudo && a_sel->content.pseudo &&
                             a_sel->content.pseudo->name && a_sel->content.pseudo->name->stryng && a_node,
                         CR_BAD_PARAM_ERROR);
    ...
    if (!a_sel->content.pseudo->extra || !a_sel->content.pseudo->extra->stryng ||
    ...
    for (; node; node = get_next_parent_element_node(node)) {
        ...
        if (val && !strqcmp((const char*)val, a_sel->content.pseudo->extra->stryng->str,
...
static gboolean pseudo_class_add_sel_matches_node(CRSelEng* a_this, CRAdditionalSel* a_add_sel, xmlNode* a_node) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_add_sel && a_add_sel->content.pseudo &&
                             a_add_sel->content.pseudo->name && a_add_sel->content.pseudo->name->stryng &&
                             a_add_sel->content.pseudo->name->stryng->str && a_node,
                         CR_BAD_PARAM_ERROR);

    status = cr_sel_eng_get_pseudo_class_selector_handler(a_this, (guchar*)a_add_sel->content.pseudo->name->stryng->str,
...
static gboolean class_add_sel_matches_node(CRAdditionalSel* a_add_sel, xmlNode* a_node) {
    ...
    g_return_val_if_fail(a_add_sel && a_add_sel->type == CLASS_ADD_SELECTOR && a_add_sel->content.class_name &&
                             a_add_sel->content.class_name->stryng && a_add_sel->content.class_name->stryng->str &&
                             a_node,
                         FALSE);
    ...
    if (xmlHasProp(a_node, (const xmlChar*)"class")) {
        ...
        for (cur = klass; cur && *cur; cur++) {
            ...
            if (!strncmp((const char*)cur, a_add_sel->content.class_name->stryng->str,
                         a_add_sel->content.class_name->stryng->len)) {
                cur += a_add_sel->content.class_name->stryng->len;
                if ((cur && !*cur) || cr_utils_is_white_space(*cur) == TRUE)
                    result = TRUE;
            }
...
static gboolean id_add_sel_matches_node(CRAdditionalSel* a_add_sel, xmlNode* a_node) {
    ...
    g_return_val_if_fail(a_add_sel && a_add_sel->type == ID_ADD_SELECTOR && a_add_sel->content.id_name &&
                             a_add_sel->content.id_name->stryng && a_add_sel->content.id_name->stryng->str && a_node,
                         FALSE);
    g_return_val_if_fail(a_add_sel && a_add_sel->type == ID_ADD_SELECTOR && a_node, FALSE);
...
static gboolean attr_add_sel_matches_node(CRAdditionalSel* a_add_sel, xmlNode* a_node) {
    ...
    g_return_val_if_fail(a_add_sel && a_add_sel->type == ATTRIBUTE_ADD_SELECTOR && a_node, FALSE);

    for (cur_sel = a_add_sel->content.attr_sel; cur_sel; cur_sel = cur_sel->next) {
        switch (cur_sel->match_way) {
            case SET:
                if (!cur_sel->name || !cur_sel->name->stryng || !cur_sel->name->stryng->str)
                    return FALSE;

                if (!xmlHasProp(a_node, (const xmlChar*)cur_sel->name->stryng->str))
                    return FALSE;
                break;
            ...
            case EQUALS: {
                ...
                if (!cur_sel->name || !cur_sel->name->stryng || !cur_sel->name->stryng->str || !cur_sel->value ||
                    !cur_sel->value->stryng || !cur_sel->value->stryng->str)
                    return FALSE;

                if (!xmlHasProp(a_node, (const xmlChar*)cur_sel->name->stryng->str))
                    return FALSE;

                value = xmlGetProp(a_node, (const xmlChar*)cur_sel->name->stryng->str);

                if (value && strcmp((const char*)value, cur_sel->value->stryng->str)) {
                    xmlFree(value);
                    return FALSE;
                }
                xmlFree(value);
            ...
            case INCLUDES: {
                xmlChar *value = NULL, *ptr1 = NULL, *ptr2 = NULL, *cur = NULL;
                ...
                for (cur = value; *cur; cur++) {
                     ...
                     */
                    while (cr_utils_is_white_space(*cur) == FALSE && *cur)
                        cur++;
                    cur--;
                    ptr2 = cur;

                    if (!strncmp((const char*)ptr1, cur_sel->value->stryng->str, ptr2 - ptr1 + 1)) {
                        found = TRUE;
                        break;
                    }
                    ptr1 = ptr2 = NULL;
            ...
            case DASHMATCH: {
                ...
                for (cur = value; *cur; cur++) {
                    ...
                    if (g_strstr_len((const gchar*)ptr1, ptr2 - ptr1 + 1, cur_sel->value->stryng->str) ==
                        (gchar*)ptr1) {
                        found = TRUE;
                        break;
                    }
 ...
 */
static gboolean additional_selector_matches_node(CRSelEng* a_this, CRAdditionalSel* a_add_sel, xmlNode* a_node) {
    CRAdditionalSel *cur_add_sel = NULL, *tail = NULL;
    gboolean evaluated = FALSE;

    for (tail = a_add_sel; tail && tail->next; tail = tail->next)
        ;

    g_return_val_if_fail(tail, FALSE);

    for (cur_add_sel = tail; cur_add_sel; cur_add_sel = cur_add_sel->prev) {
        evaluated = TRUE;
        if (cur_add_sel->type == NO_ADD_SELECTOR) {
            return FALSE;
        }

        if (cur_add_sel->type == CLASS_ADD_SELECTOR && cur_add_sel->content.class_name &&
            cur_add_sel->content.class_name->stryng && cur_add_sel->content.class_name->stryng->str) {
            if (class_add_sel_matches_node(cur_add_sel, a_node) == FALSE) {
                return FALSE;
            }
            continue;
        }
        else if (cur_add_sel->type == ID_ADD_SELECTOR && cur_add_sel->content.id_name &&
                 cur_add_sel->content.id_name->stryng && cur_add_sel->content.id_name->stryng->str) {
            if (id_add_sel_matches_node(cur_add_sel, a_node) == FALSE) {
                return FALSE;
            }
            continue;
        }
        else if (cur_add_sel->type == ATTRIBUTE_ADD_SELECTOR && cur_add_sel->content.attr_sel) {
            /*
             *here, call a function that does the match
             *against an attribute additionnal selector
             *and an xml node.
             */
            if (attr_add_sel_matches_node(cur_add_sel, a_node) == FALSE) {
                return FALSE;
            }
            continue;
        }
        else if (cur_add_sel->type == PSEUDO_CLASS_ADD_SELECTOR && cur_add_sel->content.pseudo) {
            if (pseudo_class_add_sel_matches_node(a_this, cur_add_sel, a_node) == TRUE) {
                return TRUE;
            }
            return FALSE;
        }
...
static enum CRStatus sel_matches_node_real(CRSelEng* a_this,
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_this && a_node && a_result, CR_BAD_PARAM_ERROR);
    ...
    *a_result = FALSE;
    ...
    for (cur_node = a_node; cur_sel; cur_sel = cur_sel->prev) {
        if (((cur_sel->type_mask & TYPE_SELECTOR) &&
             (cur_sel->name && cur_sel->name->stryng && cur_sel->name->stryng->str) &&
             (!strcmp(cur_sel->name->stryng->str, (const char*)cur_node->name))) ||
        ...
        if (!(cur_sel->type_mask & TYPE_SELECTOR) && !(cur_sel->type_mask & UNIVERSAL_SELECTOR)) {
            if (!cur_sel->add_sel) {
                goto done;
            }
            if (additional_selector_matches_node(a_this, cur_sel->add_sel, cur_node) == TRUE) {
        ...
        switch (cur_sel->combinator) {
            ...
            case COMB_WS: /*descendant selector */
            {
                ...
                for (n = cur_node->parent; n; n = n->parent) {
                    status = sel_matches_node_real(a_this, cur_sel->prev, n, &matches, FALSE, TRUE);
...
static enum CRStatus cr_sel_eng_get_matched_rulesets_real(CRSelEng* a_this,
    ...
    g_return_val_if_fail(a_this && a_stylesheet && a_node && a_rulesets, CR_BAD_PARAM_ERROR);
     ...
     *selectors lists.
     */
    for (cur_stmt = PRIVATE(a_this)->cur_stmt, i = 0; (PRIVATE(a_this)->cur_stmt = cur_stmt);
        ...
        switch (cur_stmt->type) {
            ...
            case AT_MEDIA_RULE_STMT:
                if (cur_stmt->kind.media_rule && cur_stmt->kind.media_rule->rulesets &&
        ...
        for (cur_sel = sel_list; cur_sel; cur_sel = cur_sel->next) {
            ...
            status = cr_sel_eng_matches_node(a_this, cur_sel->simple_sel, a_node, &matches);
...
static enum CRStatus put_css_properties_in_props_list(CRPropList** a_props, CRStatement* a_stmt) {
    CRPropList *props = NULL, *pair = NULL, *tmp_props = NULL;
    CRDeclaration* cur_decl = NULL;

    g_return_val_if_fail(a_props && a_stmt && a_stmt->type == RULESET_STMT && a_stmt->kind.ruleset, CR_BAD_PARAM_ERROR);

    props = *a_props;

    for (cur_decl = a_stmt->kind.ruleset->decl_list; cur_decl; cur_decl = cur_decl->next) {
        CRDeclaration* decl;

        decl = NULL;
        pair = NULL;

        if (!cur_decl->property || !cur_decl->property->stryng || !cur_decl->property->stryng->str)
            continue;
        /*
        ...
        if (!pair) {
            tmp_props = cr_prop_list_append2(props, cur_decl->property, cur_decl);
            if (tmp_props) {
                props = tmp_props;
                tmp_props = NULL;
            }
         ...
         */
        if (decl->parent_statement && decl->parent_statement->parent_sheet &&
            (decl->parent_statement->parent_sheet->origin < a_stmt->parent_sheet->origin)) {
            /*
             ...
             *has an UA origin)
             */
            if (decl->important == TRUE && decl->parent_statement->parent_sheet->origin != ORIGIN_UA) {
                continue;
            }
            ...
            props = tmp_props;
            tmp_props = NULL;
            props = cr_prop_list_append2(props, cur_decl->property, cur_decl);

            continue;
        }
        else if (decl->parent_statement && decl->parent_statement->parent_sheet &&
                 (decl->parent_statement->parent_sheet->origin > a_stmt->parent_sheet->origin)) {
            cr_utils_trace_info("We should not reach this line\n");
            continue;
        }
         ...
         */
        if (a_stmt->specificity >= decl->parent_statement->specificity) {
...
CRSelEng* cr_sel_eng_new(void) {
    ...
    cr_sel_eng_register_pseudo_class_sel_handler(result, (guchar*)"first-child", IDENT_PSEUDO,
                                                 (CRPseudoClassSelectorHandler)first_child_pseudo_class_handler);
    cr_sel_eng_register_pseudo_class_sel_handler(result, (guchar*)"lang", FUNCTION_PSEUDO,
                                                 (CRPseudoClassSelectorHandler)lang_pseudo_class_handler);
...
enum CRStatus cr_sel_eng_register_pseudo_class_sel_handler(CRSelEng* a_this,
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_handler && a_name, CR_BAD_PARAM_ERROR);

    handler_entry = g_try_malloc(sizeof(struct CRPseudoClassSelHandlerEntry));
    if (!handler_entry) {
...
enum CRStatus cr_sel_eng_unregister_pseudo_class_sel_handler(CRSelEng* a_this,
    ...
    for (elem = PRIVATE(a_this)->pcs_handlers; elem; elem = g_list_next(elem)) {
        entry = elem->data;
        if (!strcmp((const char*)entry->name, (const char*)a_name) && entry->type == a_type) {
            found = TRUE;
            break;
        }
    }
    ...
    PRIVATE(a_this)->pcs_handlers = g_list_delete_link(PRIVATE(a_this)->pcs_handlers, elem);
    entry = elem->data;
    if (entry->name)
        g_free(entry->name);
    g_free(elem);
...
enum CRStatus cr_sel_eng_get_pseudo_class_selector_handler(CRSelEng* a_this,
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_name, CR_BAD_PARAM_ERROR);

    for (elem = PRIVATE(a_this)->pcs_handlers; elem; elem = g_list_next(elem)) {
        entry = elem->data;
        if (!strcmp((const char*)a_name, (const char*)entry->name) && entry->type == a_type) {
            found = TRUE;
            break;
        }
    }
...
enum CRStatus cr_sel_eng_matches_node(CRSelEng* a_this, CRSimpleSel* a_sel, xmlNode* a_node, gboolean* a_result) {
    ...
    return sel_matches_node_real(a_this, a_sel, a_node, a_result, TRUE, TRUE);
}
...
enum CRStatus cr_sel_eng_get_matched_rulesets(CRSelEng* a_this,
    ...
    g_return_val_if_fail(a_this && a_sheet && a_node && a_rulesets && *a_rulesets == NULL && a_len, CR_BAD_PARAM_ERROR);
    ...
    while ((status = cr_sel_eng_get_matched_rulesets_real(a_this, a_sheet, a_node, stmts_tab + index, &tab_len)) ==
           CR_OUTPUT_TOO_SHORT_ERROR) {
        stmts_tab = g_try_realloc(stmts_tab, (tab_size + stmts_chunck_size) * sizeof(CRStatement*));
        if (!stmts_tab) {
            cr_utils_trace_info("Out of memory");
            status = CR_ERROR;
            goto error;
...
enum CRStatus cr_sel_eng_get_matched_properties_from_cascade(CRSelEng* a_this,
    ...
    g_return_val_if_fail(a_this && a_cascade && a_node && a_props, CR_BAD_PARAM_ERROR);
    ...
    for (origin = ORIGIN_UA; origin < NB_ORIGINS; origin++) {
        ...
        while ((status = cr_sel_eng_get_matched_rulesets_real(a_this, sheet, a_node, stmts_tab + index, &tab_len)) ==
               CR_OUTPUT_TOO_SHORT_ERROR) {
            stmts_tab = g_try_realloc(stmts_tab, (tab_size + stmts_chunck_size) * sizeof(CRStatement*));
            if (!stmts_tab) {
                cr_utils_trace_info("Out of memory");
        ...
        switch (stmt->type) {
            case RULESET_STMT:
                if (!stmt->parent_sheet)
                    continue;
                status = put_css_properties_in_props_list(a_props, stmt);
                break;
            default:
                break;
        }
...
enum CRStatus cr_sel_eng_get_matched_style(CRSelEng* a_this,
    ...
    g_return_val_if_fail(a_this && a_cascade && a_node && a_style, CR_BAD_PARAM_ERROR);

    status = cr_sel_eng_get_matched_properties_from_cascade(a_this, a_cascade, a_node, &props);
...
void cr_sel_eng_destroy(CRSelEng* a_this) {
    ...
    if (!PRIVATE(a_this))
        goto end;
    if (PRIVATE(a_this)->pcs_handlers) {
        cr_sel_eng_unregister_all_pseudo_class_sel_handlers(a_this);
        PRIVATE(a_this)->pcs_handlers = NULL;
    }
