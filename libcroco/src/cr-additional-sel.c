...
void cr_additional_sel_set_pseudo(CRAdditionalSel* a_this, CRPseudo* a_pseudo) {
    g_return_if_fail(a_this && a_this->type == PSEUDO_CLASS_ADD_SELECTOR);
...
CRAdditionalSel* cr_additional_sel_append(CRAdditionalSel* a_this, CRAdditionalSel* a_sel) {
    ...
    for (cur_sel = a_this; cur_sel && cur_sel->next; cur_sel = cur_sel->next)
        ;
        ...
        switch (cur->type) {
            case CLASS_ADD_SELECTOR: {
                ...
                if (cur->content.class_name) {
                    name =
                        (guchar*)g_strndup(cur->content.class_name->stryng->str, cur->content.class_name->stryng->len);
            ...
            case ID_ADD_SELECTOR: {
                ...
                if (cur->content.id_name) {
                    name = (guchar*)g_strndup(cur->content.id_name->stryng->str, cur->content.id_name->stryng->len);
            ...
            case PSEUDO_CLASS_ADD_SELECTOR: {
                if (cur->content.pseudo) {
                    ...
                    tmp_str = cr_pseudo_to_string(cur->content.pseudo);
                    if (tmp_str) {
            ...
            case ATTRIBUTE_ADD_SELECTOR:
                if (cur->content.attr_sel) {
                    ...
                    tmp_str = cr_attr_sel_to_string(cur->content.attr_sel);
                    if (tmp_str) {
                        g_string_append_printf(str_buf, "%s]", tmp_str);
                        g_free(tmp_str);
                        tmp_str = NULL;
...
guchar* cr_additional_sel_one_to_string(CRAdditionalSel const* a_this) {
    ...
    switch (a_this->type) {
        case CLASS_ADD_SELECTOR: {
            ...
            if (a_this->content.class_name) {
                name = (guchar*)g_strndup(a_this->content.class_name->stryng->str,
        ...
        case ID_ADD_SELECTOR: {
            ...
            if (a_this->content.id_name) {
                name = (guchar*)g_strndup(a_this->content.id_name->stryng->str, a_this->content.id_name->stryng->len);
        ...
        case PSEUDO_CLASS_ADD_SELECTOR: {
            if (a_this->content.pseudo) {
                ...
                tmp_str = cr_pseudo_to_string(a_this->content.pseudo);
                if (tmp_str) {
                    g_string_append_printf(str_buf, ":%s", tmp_str);
                    g_free(tmp_str);
                    tmp_str = NULL;
        ...
        case ATTRIBUTE_ADD_SELECTOR:
            if (a_this->content.attr_sel) {
                ...
                tmp_str = cr_attr_sel_to_string(a_this->content.attr_sel);
                if (tmp_str) {
                    g_string_append_printf(str_buf, "%s]", tmp_str);
                    g_free(tmp_str);
                    tmp_str = NULL;
