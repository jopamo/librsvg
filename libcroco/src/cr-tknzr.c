...
#define RECORD_INITIAL_POS(a_tknzr, a_pos)                         \
    status = cr_input_get_cur_pos(PRIVATE(a_tknzr)->input, a_pos); \
...
#define RECORD_CUR_BYTE_ADDR(a_tknzr, a_addr)                             \
    status = cr_input_get_cur_byte_addr(PRIVATE(a_tknzr)->input, a_addr); \
    CHECK_PARSING_STATUS(status, TRUE)
...
#define PEEK_BYTE(a_tknzr, a_offset, a_byte_ptr)                \
    status = cr_tknzr_peek_byte(a_tknzr, a_offset, a_byte_ptr); \
    CHECK_PARSING_STATUS(status, TRUE);
...
#define READ_NEXT_BYTE(a_tknzr, a_byte_ptr)                           \
    status = cr_input_read_byte(PRIVATE(a_tknzr)->input, a_byte_ptr); \
    CHECK_PARSING_STATUS(status, TRUE);
...
#define SKIP_BYTES(a_tknzr, a_nb_bytes)                                             \
    status = cr_input_seek_index(PRIVATE(a_tknzr)->input, CR_SEEK_CUR, a_nb_bytes); \
    CHECK_PARSING_STATUS(status, TRUE);
...
#define SKIP_CHARS(a_tknzr, a_nb_chars)                                         \
    {                                                                           \
        ...
        status = cr_input_consume_chars(PRIVATE(a_tknzr)->input, 0, &nb_chars); \
        CHECK_PARSING_STATUS(status, TRUE);                                     \
    }
...
static enum CRStatus cr_tknzr_parse_w(CRTknzr* a_this,
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && PRIVATE(a_this)->input && a_start && a_end, CR_BAD_PARAM_ERROR);
    ...
    *a_end = NULL;
    ...
    if (a_location) {
        cr_tknzr_get_parsing_location(a_this, a_location);
    }
...
static enum CRStatus cr_tknzr_parse_nl(CRTknzr* a_this,
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_start && a_end, CR_BAD_PARAM_ERROR);
...
static enum CRStatus cr_tknzr_try_to_skip_spaces(CRTknzr* a_this) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && PRIVATE(a_this)->input, CR_BAD_PARAM_ERROR);
    ...
    if (cr_utils_is_white_space(cur_char) == TRUE) {
        ...
        status = cr_input_consume_white_spaces(PRIVATE(a_this)->input, &nb_chars);
    }
...
static enum CRStatus cr_tknzr_parse_comment(CRTknzr* a_this, CRString** a_comment) {
    ...
    if (status == CR_OK) {
        cr_parsing_location_copy(&comment->location, &loc);
        *a_comment = comment;
        return CR_OK;
    }
error:
...
static enum CRStatus cr_tknzr_parse_unicode_escape(CRTknzr* a_this, guint32* a_unicode, CRParsingLocation* a_location) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_unicode, CR_BAD_PARAM_ERROR);
    ...
    for (occur = 0, unicode = 0; ((cur_char >= '0' && cur_char <= '9') || (cur_char >= 'a' && cur_char <= 'f') ||
    ...
    cr_tknzr_parse_w(a_this, &tmp_char_ptr1, &tmp_char_ptr2, NULL);
    *a_unicode = unicode;
    return CR_OK;
...
static enum CRStatus cr_tknzr_parse_escape(CRTknzr* a_this, guint32* a_esc_code, CRParsingLocation* a_location) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_esc_code, CR_BAD_PARAM_ERROR);
    ...
    if ((next_chars[1] >= '0' && next_chars[1] <= '9') || (next_chars[1] >= 'a' && next_chars[1] <= 'f') ||
        (next_chars[1] >= 'A' && next_chars[1] <= 'F')) {
        status = cr_tknzr_parse_unicode_escape(a_this, a_esc_code, a_location);
    }
    else {
        /*consume the '\' char */
        READ_NEXT_CHAR(a_this, &cur_char);
...
static enum CRStatus cr_tknzr_parse_string(CRTknzr* a_this, CRString** a_str) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && PRIVATE(a_this)->input && a_str, CR_BAD_PARAM_ERROR);
    ...
    for (;;) {
        ...
        if (next_chars[0] == '\\') {
            ...
            if (next_chars[1] == '\'' || next_chars[1] == '"') {
                g_string_append_unichar(str->stryng, next_chars[1]);
                SKIP_BYTES(a_this, 2);
                status = CR_OK;
            }
            else {
                status = cr_tknzr_parse_escape(a_this, &esc_code, NULL);

                if (status == CR_OK) {
                    g_string_append_unichar(str->stryng, esc_code);
                }
            }
            ...
            if (status != CR_OK) {
                ...
                status = cr_tknzr_parse_nl(a_this, &tmp_char_ptr1, &tmp_char_ptr2, NULL);
            }

            CHECK_PARSING_STATUS(status, FALSE);
        }
        else if (strchr("\t !#$%&", next_chars[0]) || (next_chars[0] >= '(' && next_chars[0] <= '~')) {
            READ_NEXT_CHAR(a_this, &cur_char);
    ...
    if (status == CR_OK) {
        if (*a_str == NULL) {
            *a_str = str;
            ...
            (*a_str)->stryng = g_string_append_len((*a_str)->stryng, str->stryng->str, str->stryng->len);
...
static enum CRStatus cr_tknzr_parse_nmstart(CRTknzr* a_this, guint32* a_char, CRParsingLocation* a_location) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && PRIVATE(a_this)->input && a_char, CR_BAD_PARAM_ERROR);
    ...
    if (next_char == '\\') {
        status = cr_tknzr_parse_escape(a_this, a_char, a_location);

        if (status != CR_OK)
            goto error;
    }
    else if (cr_utils_is_nonascii(next_char) == TRUE || ((next_char >= 'a') && (next_char <= 'z')) ||
...
static enum CRStatus cr_tknzr_parse_nmchar(CRTknzr* a_this, guint32* a_char, CRParsingLocation* a_location) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_char, CR_BAD_PARAM_ERROR);
    ...
    if (next_char == '\\') {
        ...
        if (status != CR_OK)
            goto error;
             ...
             ((next_char >= 'A') && (next_char <= 'Z')) || ((next_char >= '0') && (next_char <= '9')) ||
             (next_char == '-') || (next_char == '_') /*'_' not allowed by the spec. */
    ) {
        READ_NEXT_CHAR(a_this, &cur_char);
        *a_char = cur_char;
        status = CR_OK;
...
static enum CRStatus cr_tknzr_parse_ident(CRTknzr* a_this, CRString** a_str) {
    ...
    if (tmp_char == '-') {
        ...
        cr_tknzr_get_parsing_location(a_this, &stringue->location);
    ...
    g_string_append_unichar(stringue->stryng, tmp_char);
    for (;;) {
        status = cr_tknzr_parse_nmchar(a_this, &tmp_char, NULL);
        if (status != CR_OK) {
            status = CR_OK;
            break;
        }
    ...
    if (status == CR_OK) {
        if (!*a_str) {
            *a_str = stringue;
            ...
            g_string_append_len((*a_str)->stryng, stringue->stryng->str, stringue->stryng->len);
...
static enum CRStatus cr_tknzr_parse_name(CRTknzr* a_this, CRString** a_str) {
    ...
    CRInputPos init_pos;
    enum CRStatus status = CR_OK;
    gboolean str_needs_free = FALSE, is_first_nmchar = TRUE;
    glong i = 0;
    CRParsingLocation loc = {0};
    ...
    for (i = 0;; i++) {
        if (is_first_nmchar == TRUE) {
            status = cr_tknzr_parse_nmchar(a_this, &tmp_char, &loc);
        ...
        if (status != CR_OK)
            break;
        g_string_append_unichar((*a_str)->stryng, tmp_char);
    }
    if (i > 0) {
...
static enum CRStatus cr_tknzr_parse_uri(CRTknzr* a_this, CRString** a_str) {
    ...
    if (status == CR_OK) {
        guint32 next_char = 0;
        status = cr_tknzr_parse_w(a_this, &tmp_ptr1, &tmp_ptr2, NULL);
        cr_tknzr_try_to_skip_spaces(a_this);
        PEEK_NEXT_CHAR(a_this, &next_char);
        if (next_char == ')') {
            READ_NEXT_CHAR(a_this, &cur_char);
    ...
    if (status != CR_OK) {
        ...
        for (;;) {
            guint32 next_char = 0;
            PEEK_NEXT_CHAR(a_this, &next_char);
            if (strchr("!#$%&", next_char) || (next_char >= '*' && next_char <= '~') ||
        ...
        if (str) {
            if (*a_str == NULL) {
                *a_str = str;
                ...
                g_string_append_len((*a_str)->stryng, str->stryng->str, str->stryng->len);
                cr_string_destroy(str);
            }
        }
    }

    cr_parsing_location_copy(&(*a_str)->location, &location);
    return CR_OK;
error:
    if (str) {
        cr_string_destroy(str);
...
static enum CRStatus cr_tknzr_parse_rgb(CRTknzr* a_this, CRRgb** a_rgb) {
    ...
    if (((next_bytes[0] == 'r') || (next_bytes[0] == 'R')) && ((next_bytes[1] == 'g') || (next_bytes[1] == 'G')) &&
...
static enum CRStatus cr_tknzr_parse_important(CRTknzr* a_this, CRParsingLocation* a_location) {
    ...
    if (BYTE(PRIVATE(a_this)->input, 1, NULL) == 'i' && BYTE(PRIVATE(a_this)->input, 2, NULL) == 'm' &&
        BYTE(PRIVATE(a_this)->input, 3, NULL) == 'p' && BYTE(PRIVATE(a_this)->input, 4, NULL) == 'o' &&
        BYTE(PRIVATE(a_this)->input, 5, NULL) == 'r' && BYTE(PRIVATE(a_this)->input, 6, NULL) == 't' &&
        BYTE(PRIVATE(a_this)->input, 7, NULL) == 'a' && BYTE(PRIVATE(a_this)->input, 8, NULL) == 'n' &&
...
static enum CRStatus cr_tknzr_parse_num(CRTknzr* a_this, CRNum** a_num) {
    ...
    if (status == CR_OK) {
        ...
        cr_parsing_location_copy(&(*a_num)->location, &location);
        return CR_OK;
    }
...
CRTknzr* cr_tknzr_new_from_buf(guchar* a_buf, gulong a_len, enum CREncoding a_enc, gboolean a_free_at_destroy) {
    ...
    input = cr_input_new_from_buf(a_buf, a_len, a_enc, a_free_at_destroy);
...
enum CRStatus cr_tknzr_read_char(CRTknzr* a_this, guint32* a_char) {
    ...
    if (PRIVATE(a_this)->token_cache) {
        cr_input_set_cur_pos(PRIVATE(a_this)->input, &PRIVATE(a_this)->prev_pos);
...
enum CRStatus cr_tknzr_peek_byte(CRTknzr* a_this, gulong a_offset, guchar* a_byte) {
    g_return_val_if_fail(a_this && PRIVATE(a_this) && PRIVATE(a_this)->input && a_byte, CR_BAD_PARAM_ERROR);
    ...
    return cr_input_peek_byte(PRIVATE(a_this)->input, CR_SEEK_CUR, a_offset, a_byte);
}
...
guchar cr_tknzr_peek_byte2(CRTknzr* a_this, gulong a_offset, gboolean* a_eof) {
    g_return_val_if_fail(a_this && PRIVATE(a_this) && PRIVATE(a_this)->input, 0);
...
enum CRStatus cr_tknzr_get_cur_pos(CRTknzr* a_this, CRInputPos* a_pos) {
    g_return_val_if_fail(a_this && PRIVATE(a_this) && PRIVATE(a_this)->input && a_pos, CR_BAD_PARAM_ERROR);
...
enum CRStatus cr_tknzr_get_parsing_location(CRTknzr* a_this, CRParsingLocation* a_loc) {
    ...
    return cr_input_get_parsing_location(PRIVATE(a_this)->input, a_loc);
}
...
enum CRStatus cr_tknzr_consume_chars(CRTknzr* a_this, guint32 a_char, glong* a_nb_char) {
    ...
    status = cr_input_consume_chars(PRIVATE(a_this)->input, a_char, &consumed);
...
enum CRStatus cr_tknzr_unget_token(CRTknzr* a_this, CRToken* a_token) {
    g_return_val_if_fail(a_this && PRIVATE(a_this) && PRIVATE(a_this)->token_cache == NULL, CR_BAD_PARAM_ERROR);
...
enum CRStatus cr_tknzr_get_next_token(CRTknzr* a_this, CRToken** a_tk) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_tk && *a_tk == NULL && PRIVATE(a_this)->input,
                         CR_BAD_PARAM_ERROR);
    ...
    status = cr_input_get_end_of_file(PRIVATE(a_this)->input, &reached_eof);
    ...
    switch (next_char) {
        case '@': {
            if (BYTE(input, 2, NULL) == 'f' && BYTE(input, 3, NULL) == 'o' && BYTE(input, 4, NULL) == 'n' &&
                BYTE(input, 5, NULL) == 't' && BYTE(input, 6, NULL) == '-' && BYTE(input, 7, NULL) == 'f' &&
                BYTE(input, 8, NULL) == 'a' && BYTE(input, 9, NULL) == 'c' && BYTE(input, 10, NULL) == 'e') {
                SKIP_CHARS(a_this, 1);
                cr_tknzr_get_parsing_location(a_this, &location);
                SKIP_CHARS(a_this, 9);
                status = cr_token_set_font_face_sym(token);
                CHECK_PARSING_STATUS(status, TRUE);
                cr_parsing_location_copy(&token->location, &location);
                goto done;
            }

            if (BYTE(input, 2, NULL) == 'c' && BYTE(input, 3, NULL) == 'h' && BYTE(input, 4, NULL) == 'a' &&
                BYTE(input, 5, NULL) == 'r' && BYTE(input, 6, NULL) == 's' && BYTE(input, 7, NULL) == 'e' &&
                BYTE(input, 8, NULL) == 't') {
                SKIP_CHARS(a_this, 1);
                cr_tknzr_get_parsing_location(a_this, &location);
                SKIP_CHARS(a_this, 7);
            ...
            if (BYTE(input, 2, NULL) == 'i' && BYTE(input, 3, NULL) == 'm' && BYTE(input, 4, NULL) == 'p' &&
                BYTE(input, 5, NULL) == 'o' && BYTE(input, 6, NULL) == 'r' && BYTE(input, 7, NULL) == 't') {
                SKIP_CHARS(a_this, 1);
                cr_tknzr_get_parsing_location(a_this, &location);
                SKIP_CHARS(a_this, 6);
            ...
            if (BYTE(input, 2, NULL) == 'm' && BYTE(input, 3, NULL) == 'e' && BYTE(input, 4, NULL) == 'd' &&
                BYTE(input, 5, NULL) == 'i' && BYTE(input, 6, NULL) == 'a') {
                SKIP_CHARS(a_this, 1);
                cr_tknzr_get_parsing_location(a_this, &location);
                SKIP_CHARS(a_this, 5);
            ...
            if (BYTE(input, 2, NULL) == 'p' && BYTE(input, 3, NULL) == 'a' && BYTE(input, 4, NULL) == 'g' &&
                BYTE(input, 5, NULL) == 'e') {
                SKIP_CHARS(a_this, 1);
                cr_tknzr_get_parsing_location(a_this, &location);
                SKIP_CHARS(a_this, 4);
            ...
            if (status == CR_OK) {
                ...
                CHECK_PARSING_STATUS(status, TRUE);
                if (str) {
                    cr_parsing_location_copy(&token->location, &str->location);
                }
                goto done;
            }
        } break;
        ...
        case 'u':
            ...
            if (BYTE(input, 2, NULL) == 'r' && BYTE(input, 3, NULL) == 'l' && BYTE(input, 4, NULL) == '(') {
                CRString* str2 = NULL;
                ...
                if (status == CR_OK) {
                    ...
                    CHECK_PARSING_STATUS(status, TRUE);
                    if (str2) {
                        cr_parsing_location_copy(&token->location, &str2->location);
                    }
                    goto done;
                }
            }
        ...
        case 'r':
            if (BYTE(input, 2, NULL) == 'g' && BYTE(input, 3, NULL) == 'b' && BYTE(input, 4, NULL) == '(') {
                status = cr_tknzr_parse_rgb(a_this, &rgb);
                if (status == CR_OK && rgb) {
                    status = cr_token_set_rgb(token, rgb);
                    CHECK_PARSING_STATUS(status, TRUE);
                    if (rgb) {
                        cr_parsing_location_copy(&token->location, &rgb->location);
                    }
                    rgb = NULL;
                    goto done;
                }
        ...
        case '<':
            if (BYTE(input, 2, NULL) == '!' && BYTE(input, 3, NULL) == '-' && BYTE(input, 4, NULL) == '-') {
                SKIP_CHARS(a_this, 1);
                cr_tknzr_get_parsing_location(a_this, &location);
                SKIP_CHARS(a_this, 3);
        ...
        case '-':
            if (BYTE(input, 2, NULL) == '-' && BYTE(input, 3, NULL) == '>') {
                SKIP_CHARS(a_this, 1);
                cr_tknzr_get_parsing_location(a_this, &location);
                SKIP_CHARS(a_this, 2);
        ...
        case '\r': {
            ...
            status = cr_tknzr_parse_w(a_this, &start, &end, &location);
            if (status == CR_OK) {
        ...
        parse_number: {
            ...
            if (status == CR_OK && num) {
                ...
                if (next_bytes[0] == 'e' && next_bytes[1] == 'm') {
                    num->type = NUM_LENGTH_EM;
                    status = cr_token_set_ems(token, num);
                    num = NULL;
                    SKIP_CHARS(a_this, 2);
                }
                else if (next_bytes[0] == 'e' && next_bytes[1] == 'x') {
                    num->type = NUM_LENGTH_EX;
                    status = cr_token_set_exs(token, num);
                    num = NULL;
                    SKIP_CHARS(a_this, 2);
                }
                else if (next_bytes[0] == 'p' && next_bytes[1] == 'x') {
                    num->type = NUM_LENGTH_PX;
                    status = cr_token_set_length(token, num, LENGTH_PX_ET);
                    num = NULL;
                    SKIP_CHARS(a_this, 2);
                }
                else if (next_bytes[0] == 'c' && next_bytes[1] == 'm') {
                    num->type = NUM_LENGTH_CM;
                    status = cr_token_set_length(token, num, LENGTH_CM_ET);
                    num = NULL;
                    SKIP_CHARS(a_this, 2);
                }
                else if (next_bytes[0] == 'm' && next_bytes[1] == 'm') {
                    num->type = NUM_LENGTH_MM;
                    status = cr_token_set_length(token, num, LENGTH_MM_ET);
                    num = NULL;
                    SKIP_CHARS(a_this, 2);
                }
                else if (next_bytes[0] == 'i' && next_bytes[1] == 'n') {
                    num->type = NUM_LENGTH_IN;
                    status = cr_token_set_length(token, num, LENGTH_IN_ET);
                    num = NULL;
                    SKIP_CHARS(a_this, 2);
                }
                else if (next_bytes[0] == 'p' && next_bytes[1] == 't') {
                    num->type = NUM_LENGTH_PT;
                    status = cr_token_set_length(token, num, LENGTH_PT_ET);
                    num = NULL;
                    SKIP_CHARS(a_this, 2);
                }
                else if (next_bytes[0] == 'p' && next_bytes[1] == 'c') {
                    num->type = NUM_LENGTH_PC;
                    status = cr_token_set_length(token, num, LENGTH_PC_ET);
                    num = NULL;
                    SKIP_CHARS(a_this, 2);
                }
                else if (next_bytes[0] == 'd' && next_bytes[1] == 'e' && next_bytes[2] == 'g') {
                    num->type = NUM_ANGLE_DEG;
                    status = cr_token_set_angle(token, num, ANGLE_DEG_ET);
                    num = NULL;
                    SKIP_CHARS(a_this, 3);
                }
                else if (next_bytes[0] == 'r' && next_bytes[1] == 'a' && next_bytes[2] == 'd') {
                    num->type = NUM_ANGLE_RAD;
                    status = cr_token_set_angle(token, num, ANGLE_RAD_ET);
                    num = NULL;
                    SKIP_CHARS(a_this, 3);
                }
                else if (next_bytes[0] == 'g' && next_bytes[1] == 'r' && next_bytes[2] == 'a' && next_bytes[3] == 'd') {
                    num->type = NUM_ANGLE_GRAD;
                    status = cr_token_set_angle(token, num, ANGLE_GRAD_ET);
                    num = NULL;
                    SKIP_CHARS(a_this, 4);
                }
                else if (next_bytes[0] == 'm' && next_bytes[1] == 's') {
                    num->type = NUM_TIME_MS;
                    status = cr_token_set_time(token, num, TIME_MS_ET);
                    num = NULL;
                    SKIP_CHARS(a_this, 2);
                }
                else if (next_bytes[0] == 's') {
                    num->type = NUM_TIME_S;
                    status = cr_token_set_time(token, num, TIME_S_ET);
                    num = NULL;
                    SKIP_CHARS(a_this, 1);
                }
                else if (next_bytes[0] == 'H' && next_bytes[1] == 'z') {
                    num->type = NUM_FREQ_HZ;
                    status = cr_token_set_freq(token, num, FREQ_HZ_ET);
                    num = NULL;
                    SKIP_CHARS(a_this, 2);
                }
                else if (next_bytes[0] == 'k' && next_bytes[1] == 'H' && next_bytes[2] == 'z') {
                    num->type = NUM_FREQ_KHZ;
                    status = cr_token_set_freq(token, num, FREQ_KHZ_ET);
                    num = NULL;
                    SKIP_CHARS(a_this, 3);
                }
                else if (next_bytes[0] == '%') {
                    num->type = NUM_PERCENTAGE;
                    status = cr_token_set_percentage(token, num);
                    num = NULL;
                    SKIP_CHARS(a_this, 1);
                }
                else {
                    ...
                    if (status == CR_OK && str) {
                        ...
                        status = cr_token_set_dimen(token, num, str);
                        num = NULL;
                        CHECK_PARSING_STATUS(status, TRUE);
                        str = NULL;
                    }
                    else {
                        status = cr_token_set_number(token, num);
                        num = NULL;
                ...
                if (token && token->u.num) {
                    cr_parsing_location_copy(&token->location, &token->u.num->location);
                }
                else {
                    status = CR_ERROR;
                }
        ...
        fallback:
            ...
            if (next_char == '\\' || (cr_utils_is_nonascii(next_bytes[0]) == TRUE) ||
                ((next_char >= 'a') && (next_char <= 'z')) || ((next_char >= 'A') && (next_char <= 'Z'))) {
                status = cr_tknzr_parse_ident(a_this, &str);
                if (status == CR_OK && str) {
                    guint32 next_c = 0;

                    status = cr_input_peek_char(PRIVATE(a_this)->input, &next_c);

                    if (status == CR_OK && next_c == '(') {
                        SKIP_CHARS(a_this, 1);
                        status = cr_token_set_function(token, str);
                        CHECK_PARSING_STATUS(status, TRUE);
    ...
    if (status == CR_OK && token) {
        *a_tk = token;
         ...
         */
        memmove(&PRIVATE(a_this)->prev_pos, &init_pos, sizeof(CRInputPos));
        return CR_OK;
    }
...
enum CRStatus cr_tknzr_parse_token(CRTknzr* a_this,
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && PRIVATE(a_this)->input && a_res, CR_BAD_PARAM_ERROR);
...
void cr_tknzr_destroy(CRTknzr* a_this) {
    ...
    if (PRIVATE(a_this) && PRIVATE(a_this)->input) {
        if (cr_input_unref(PRIVATE(a_this)->input) == TRUE) {
