...
CRInput* cr_input_new_from_buf(guchar* a_buf, gulong a_len, enum CREncoding a_enc, gboolean a_free_buf) {
    ...
    if (a_enc != CR_UTF_8) {
        ...
        status = cr_enc_handler_convert_input(enc_handler, a_buf, &len, &PRIVATE(result)->in_buf,
...
CRInput* cr_input_new_from_uri(const gchar* a_file_uri, enum CREncoding a_enc) {
    ...
    /*load the file */
    while (loop) {
        nb_read = fread(tmp_buf, 1 /*read bytes */, CR_INPUT_MEM_CHUNK_SIZE /*nb of bytes */, file_ptr);
...
enum CRStatus cr_input_end_of_input(CRInput const* a_this, gboolean* a_end_of_input) {
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_end_of_input, CR_BAD_PARAM_ERROR);

    *a_end_of_input = (PRIVATE(a_this)->next_byte_index >= PRIVATE(a_this)->in_buf_size) ? TRUE : FALSE;
...
glong cr_input_get_nb_bytes_left(CRInput const* a_this) {
    ...
    g_return_val_if_fail(PRIVATE(a_this)->nb_bytes <= PRIVATE(a_this)->in_buf_size, -1);
    g_return_val_if_fail(PRIVATE(a_this)->next_byte_index <= PRIVATE(a_this)->nb_bytes, -1);
...
enum CRStatus cr_input_read_byte(CRInput* a_this, guchar* a_byte) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_byte, CR_BAD_PARAM_ERROR);
    ...
    *a_byte = PRIVATE(a_this)->in_buf[PRIVATE(a_this)->next_byte_index];
    ...
    if (PRIVATE(a_this)->nb_bytes - PRIVATE(a_this)->next_byte_index < 2) {
...
enum CRStatus cr_input_read_char(CRInput* a_this, guint32* a_char) {
    ...
    status = cr_utils_read_char_from_utf8_buf(PRIVATE(a_this)->in_buf + PRIVATE(a_this)->next_byte_index, nb_bytes_left,
                                              a_char, &consumed);
...
enum CRStatus cr_input_get_line_num(CRInput const* a_this, glong* a_line_num) {
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_line_num, CR_BAD_PARAM_ERROR);
...
enum CRStatus cr_input_get_column_num(CRInput const* a_this, glong* a_col) {
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_col, CR_BAD_PARAM_ERROR);
...
enum CRStatus cr_input_consume_chars(CRInput* a_this, guint32 a_char, gulong* a_nb_char) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_nb_char, CR_BAD_PARAM_ERROR);

    g_return_val_if_fail(a_char != 0 || a_nb_char != NULL, CR_BAD_PARAM_ERROR);

    for (nb_consumed = 0; ((status == CR_OK) && (*a_nb_char > 0 && nb_consumed < *a_nb_char)); nb_consumed++) {
        status = cr_input_consume_char(a_this, a_char);
    }

    *a_nb_char = nb_consumed;

    if ((nb_consumed > 0) && ((status == CR_PARSING_ERROR) || (status == CR_END_OF_INPUT_ERROR))) {
        status = CR_OK;
    }
...
enum CRStatus cr_input_consume_white_spaces(CRInput* a_this, gulong* a_nb_chars) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_nb_chars, CR_BAD_PARAM_ERROR);
...
enum CRStatus cr_input_peek_char(CRInput const* a_this, guint32* a_char) {
    ...
    if (PRIVATE(a_this)->next_byte_index >= PRIVATE(a_this)->in_buf_size) {
        return CR_END_OF_INPUT_ERROR;
    }
...
guchar cr_input_peek_byte2(CRInput const* a_this, gulong a_offset, gboolean* a_eof) {
    ...
    if ((status == CR_END_OF_INPUT_ERROR) && a_eof)
        *a_eof = TRUE;
...
enum CRStatus cr_input_get_cur_byte_addr(CRInput* a_this, guchar** a_offset) {
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_offset, CR_BAD_PARAM_ERROR);

    if (!PRIVATE(a_this)->next_byte_index) {
        return CR_START_OF_INPUT_ERROR;
    }

    *a_offset = cr_input_get_byte_addr(a_this, PRIVATE(a_this)->next_byte_index - 1);
...
enum CRStatus cr_input_seek_index(CRInput* a_this, enum CRSeekPos a_origin, gint a_pos) {
    ...
    if ((abs_offset > 0) && (gulong)abs_offset < PRIVATE(a_this)->nb_bytes) {
...
enum CRStatus cr_input_get_cur_pos(CRInput const* a_this, CRInputPos* a_pos) {
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_pos, CR_BAD_PARAM_ERROR);
...
enum CRStatus cr_input_get_cur_index(CRInput const* a_this, glong* a_index) {
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_index, CR_BAD_PARAM_ERROR);
...
enum CRStatus cr_input_get_end_of_file(CRInput const* a_this, gboolean* a_eof) {
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_eof, CR_BAD_PARAM_ERROR);
...
enum CRStatus cr_input_get_end_of_line(CRInput const* a_this, gboolean* a_eol) {
    g_return_val_if_fail(a_this && PRIVATE(a_this) && a_eol, CR_BAD_PARAM_ERROR);
