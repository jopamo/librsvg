...
static void parse_font_face_property_cb(CRDocHandler* a_this, CRString* a_name, CRTerm* a_value, gboolean a_important) {
    ...
    stmt->kind.font_face_rule->decl_list = cr_declaration_append(stmt->kind.font_face_rule->decl_list, decl);
...
static void parse_page_start_page_cb(CRDocHandler* a_this,
                                     CRString* a_name,
    ...
    stmt = cr_statement_new_at_page_rule(NULL, NULL, page_name, pseudo_name);
    page_name = NULL;
    pseudo_name = NULL;
    g_return_if_fail(stmt);
    status = cr_doc_handler_set_ctxt(a_this, stmt);
...
static void parse_page_property_cb(CRDocHandler* a_this, CRString* a_name, CRTerm* a_expression, gboolean a_important) {
    ...
    stmt->kind.page_rule->decl_list = cr_declaration_append(stmt->kind.page_rule->decl_list, decl);
...
static void parse_at_media_end_selector_cb(CRDocHandler* a_this, CRSelector* a_sellist) {
    ...
    stmtptr = &stmt;
    status = cr_doc_handler_get_ctxt(a_this, (gpointer*)stmtptr);
    g_return_if_fail(status == CR_OK && stmt && stmt->type == RULESET_STMT);
    g_return_if_fail(stmt->kind.ruleset->parent_media_rule);

    status = cr_doc_handler_set_ctxt(a_this, stmt->kind.ruleset->parent_media_rule);
    g_return_if_fail(status == CR_OK);
}
...
static void parse_ruleset_property_cb(CRDocHandler* a_this, CRString* a_name, CRTerm* a_value, gboolean a_important) {
    ...
    g_return_if_fail(status == CR_OK && ruleset && ruleset->type == RULESET_STMT);
...
static void parse_ruleset_end_selector_cb(CRDocHandler* a_this, CRSelector* a_sellist) {
    ...
    g_return_if_fail(status == CR_OK && result && result->type == RULESET_STMT);
}
    ...
    switch (a_this->type) {
        ...
        case AT_MEDIA_RULE_STMT:
            ...
            if (a_this->kind.media_rule->rulesets) {
                cr_statement_destroy(a_this->kind.media_rule->rulesets);
                a_this->kind.media_rule->rulesets = NULL;
            }
            if (a_this->kind.media_rule->media_list) {
                GList* cur = NULL;

                for (cur = a_this->kind.media_rule->media_list; cur; cur = cur->next) {
                    if (cur->data) {
        ...
        case AT_PAGE_RULE_STMT:
            ...
            if (a_this->kind.page_rule->decl_list) {
                cr_declaration_destroy(a_this->kind.page_rule->decl_list);
                a_this->kind.page_rule->decl_list = NULL;
            }
            if (a_this->kind.page_rule->name) {
                cr_string_destroy(a_this->kind.page_rule->name);
                a_this->kind.page_rule->name = NULL;
            }
            if (a_this->kind.page_rule->pseudo) {
                cr_string_destroy(a_this->kind.page_rule->pseudo);
 ...
 */
static gchar* cr_statement_ruleset_to_string(CRStatement const* a_this, glong a_indent) {
    ...
    if (a_this->kind.ruleset->sel_list) {
        ...
        tmp_str = (gchar*)cr_selector_to_string(a_this->kind.ruleset->sel_list);
        if (tmp_str) {
    ...
    if (a_this->kind.ruleset->decl_list) {
        tmp_str = (gchar*)cr_declaration_list_to_string2(a_this->kind.ruleset->decl_list,
 ...
 *using g_free().
 */
static gchar* cr_statement_font_face_rule_to_string(CRStatement const* a_this, glong a_indent) {
    gchar *result = NULL, *tmp_str = NULL;
    GString* stringue = NULL;

    g_return_val_if_fail(a_this && a_this->type == AT_FONT_FACE_RULE_STMT, NULL);
    ...
    if (a_this->kind.font_face_rule->decl_list) {
        ...
        tmp_str = (gchar*)cr_declaration_list_to_string2(a_this->kind.font_face_rule->decl_list,
                                                         a_indent + DECLARATION_INDENT_NB, TRUE);
        if (tmp_str) {
            g_string_append(stringue, tmp_str);
            g_free(tmp_str);
 ...
 */
static gchar* cr_statement_charset_to_string(CRStatement const* a_this, gulong a_indent) {
    gchar* str = NULL;
    GString* stringue = NULL;

    g_return_val_if_fail(a_this && a_this->type == AT_CHARSET_RULE_STMT, NULL);

    if (a_this->kind.charset_rule && a_this->kind.charset_rule->charset && a_this->kind.charset_rule->charset->stryng &&
        a_this->kind.charset_rule->charset->stryng->str) {
        str =
            g_strndup(a_this->kind.charset_rule->charset->stryng->str, a_this->kind.charset_rule->charset->stryng->len);
        g_return_val_if_fail(str, NULL);
        stringue = g_string_new(NULL);
        g_return_val_if_fail(stringue, NULL);
        cr_utils_dump_n_chars2(' ', stringue, a_indent);
        g_string_append_printf(stringue, "@charset \"%s\" ;", str);
        if (str) {
            g_free(str);
            str = NULL;
        }
 ...
 */
static gchar* cr_statement_at_page_rule_to_string(CRStatement const* a_this, gulong a_indent) {
    GString* stringue = NULL;
    gchar* result = NULL;
    ...
    if (a_this->kind.page_rule->name && a_this->kind.page_rule->name->stryng) {
        g_string_append_printf(stringue, " %s", a_this->kind.page_rule->name->stryng->str);
    }
    else {
        g_string_append(stringue, " ");
    }
    if (a_this->kind.page_rule->pseudo && a_this->kind.page_rule->pseudo->stryng) {
        g_string_append_printf(stringue, " :%s", a_this->kind.page_rule->pseudo->stryng->str);
    }
    if (a_this->kind.page_rule->decl_list) {
        gchar* str = NULL;
        g_string_append(stringue, " {\n");
        str = (gchar*)cr_declaration_list_to_string2(a_this->kind.page_rule->decl_list,
                                                     a_indent + DECLARATION_INDENT_NB, TRUE);
        if (str) {
 ...
 *by the caller using g_free().
 */
static gchar* cr_statement_media_rule_to_string(CRStatement const* a_this, gulong a_indent) {
    gchar* str = NULL;
    GString* stringue = NULL;
    GList const* cur = NULL;

    g_return_val_if_fail(a_this->type == AT_MEDIA_RULE_STMT, NULL);
    ...
    if (a_this->kind.media_rule) {
        ...
        for (cur = a_this->kind.media_rule->media_list; cur; cur = cur->next) {
            if (cur->data) {
                gchar* str2 = cr_string_dup2((CRString const*)cur->data);

                if (str2) {
                    if (cur->prev) {
                        g_string_append(stringue, ",");
                    }
                    g_string_append_printf(stringue, " %s", str2);
                    g_free(str2);
                    str2 = NULL;
                }
            }
        ...
        g_string_append(stringue, " {\n");
        str = cr_statement_list_to_string(a_this->kind.media_rule->rulesets, a_indent + DECLARATION_INDENT_NB);
        if (str) {
            g_string_append(stringue, str);
            g_free(str);
            str = NULL;
...
static gchar* cr_statement_import_rule_to_string(CRStatement const* a_this, gulong a_indent) {
    GString* stringue = NULL;
    gchar* str = NULL;

    g_return_val_if_fail(a_this && a_this->type == AT_IMPORT_RULE_STMT && a_this->kind.import_rule, NULL);

    if (a_this->kind.import_rule->url && a_this->kind.import_rule->url->stryng) {
        stringue = g_string_new(NULL);
        g_return_val_if_fail(stringue, NULL);
        str = g_strndup(a_this->kind.import_rule->url->stryng->str, a_this->kind.import_rule->url->stryng->len);
        cr_utils_dump_n_chars2(' ', stringue, a_indent);
        if (str) {
            g_string_append_printf(stringue, "@import url(\"%s\")", str);
            g_free(str);
            str = NULL;
        }
        ...
        if (a_this->kind.import_rule->media_list) {
            ...
            for (cur = a_this->kind.import_rule->media_list; cur; cur = cur->next) {
                if (cur->data) {
                    ...
                    if (crstr && crstr->stryng && crstr->stryng->str) {
                        g_string_append_len(stringue, crstr->stryng->str, crstr->stryng->len);
                    }
                }
            }
        }
...
gboolean cr_statement_does_buf_parses_against_core(const guchar* a_buf, enum CREncoding a_encoding) {
    ...
    parser = cr_parser_new_from_buf((guchar*)a_buf, strlen((const char*)a_buf), a_encoding, FALSE);
...
CRStatement* cr_statement_parse_from_buf(const guchar* a_buf, enum CREncoding a_encoding) {
    ...
    if (!result) {
        result = cr_statement_at_charset_rule_parse_from_buf(a_buf, a_encoding);
    }
    else {
        goto out;
    }
    ...
    if (!result) {
        result = cr_statement_at_media_rule_parse_from_buf(a_buf, a_encoding);
    }
    else {
        goto out;
    }
    ...
    if (!result) {
        result = cr_statement_font_face_rule_parse_from_buf(a_buf, a_encoding);
    }
    else {
        goto out;
    }
    ...
    if (!result) {
        result = cr_statement_at_page_rule_parse_from_buf(a_buf, a_encoding);
    }
    else {
        goto out;
    }
    ...
    if (!result) {
        result = cr_statement_at_import_rule_parse_from_buf(a_buf, a_encoding);
    }
    else {
        goto out;
    }
...
CRStatement* cr_statement_ruleset_parse_from_buf(const guchar* a_buf, enum CREncoding a_enc) {
    ...
    parser = cr_parser_new_from_buf((guchar*)a_buf, strlen((const char*)a_buf), a_enc, FALSE);
    ...
    sac_handler->unrecoverable_error = parse_ruleset_unrecoverable_error_cb;
    ...
    resultptr = &result;
    status = cr_doc_handler_get_result(sac_handler, (gpointer*)resultptr);
...
CRStatement* cr_statement_new_ruleset(CRStyleSheet* a_sheet,
    ...
    if (a_parent_media_rule) {
        g_return_val_if_fail(a_parent_media_rule->type == AT_MEDIA_RULE_STMT, NULL);
        g_return_val_if_fail(a_parent_media_rule->kind.media_rule, NULL);
    }
    ...
    if (a_parent_media_rule) {
        ...
        a_parent_media_rule->kind.media_rule->rulesets =
            cr_statement_append(a_parent_media_rule->kind.media_rule->rulesets, result);
    }
...
CRStatement* cr_statement_at_media_rule_parse_from_buf(const guchar* a_buf, enum CREncoding a_enc) {
    ...
    sac_handler = cr_doc_handler_new();
    if (!sac_handler) {
        cr_utils_trace_info("Instantiation of the sac handler failed");
        goto cleanup;
    }
    ...
    sac_handler->unrecoverable_error = parse_at_media_unrecoverable_error_cb;
...
CRStatement* cr_statement_at_import_rule_parse_from_buf(const guchar* a_buf, enum CREncoding a_encoding) {
    ...
    status = cr_parser_parse_import(parser, &media_list, &import_string, &location);
    if (status != CR_OK || !import_string)
        goto cleanup;

    result = cr_statement_new_at_import_rule(NULL, import_string, media_list, NULL);
    if (result) {
...
CRStatement* cr_statement_new_at_font_face_rule(CRStyleSheet* a_sheet, CRDeclaration* a_font_decls) {
    ...
    result->kind.font_face_rule = g_try_malloc(sizeof(CRAtFontFaceRule));
...
CRStatement* cr_statement_font_face_rule_parse_from_buf(const guchar* a_buf, enum CREncoding a_encoding) {
    ...
    sac_handler->unrecoverable_error = parse_font_face_unrecoverable_error_cb;
...
CRStatement* cr_statement_unlink(CRStatement* a_stmt) {
    ...
    if (a_stmt->parent_sheet && a_stmt->parent_sheet->statements == a_stmt) {
        a_stmt->parent_sheet->statements = a_stmt->parent_sheet->statements->next;
    }
...
enum CRStatus cr_statement_ruleset_set_sel_list(CRStatement* a_this, CRSelector* a_sel_list) {
    g_return_val_if_fail(a_this && a_this->type == RULESET_STMT, CR_BAD_PARAM_ERROR);
...
enum CRStatus cr_statement_ruleset_get_declarations(CRStatement* a_this, CRDeclaration** a_decl_list) {
    g_return_val_if_fail(a_this && a_this->type == RULESET_STMT && a_this->kind.ruleset && a_decl_list,
                         CR_BAD_PARAM_ERROR);
...
enum CRStatus cr_statement_ruleset_get_sel_list(CRStatement const* a_this, CRSelector** a_list) {
    g_return_val_if_fail(a_this && a_this->type == RULESET_STMT && a_this->kind.ruleset, CR_BAD_PARAM_ERROR);
...
enum CRStatus cr_statement_ruleset_append_decl2(CRStatement* a_this, CRString* a_prop, CRTerm* a_value) {
    ...
    new_decls = cr_declaration_append2(a_this->kind.ruleset->decl_list, a_prop, a_value);
...
enum CRStatus cr_statement_ruleset_append_decl(CRStatement* a_this, CRDeclaration* a_decl) {
    ...
    new_decls = cr_declaration_append(a_this->kind.ruleset->decl_list, a_decl);
    g_return_val_if_fail(new_decls, CR_ERROR);
    a_this->kind.ruleset->decl_list = new_decls;
...
enum CRStatus cr_statement_at_import_rule_set_imported_sheet(CRStatement* a_this, CRStyleSheet* a_sheet) {
    g_return_val_if_fail(a_this && a_this->type == AT_IMPORT_RULE_STMT && a_this->kind.import_rule, CR_BAD_PARAM_ERROR);
...
int cr_statement_at_media_nr_rules(CRStatement const* a_this) {
    g_return_val_if_fail(a_this && a_this->type == AT_MEDIA_RULE_STMT && a_this->kind.media_rule, CR_BAD_PARAM_ERROR);
...
CRStatement* cr_statement_at_media_get_from_list(CRStatement* a_this, int itemnr) {
    g_return_val_if_fail(a_this && a_this->type == AT_MEDIA_RULE_STMT && a_this->kind.media_rule, NULL);
...
enum CRStatus cr_statement_at_page_rule_set_declarations(CRStatement* a_this, CRDeclaration* a_decl_list) {
    g_return_val_if_fail(a_this && a_this->type == AT_PAGE_RULE_STMT && a_this->kind.page_rule, CR_BAD_PARAM_ERROR);
...
enum CRStatus cr_statement_at_charset_rule_set_charset(CRStatement* a_this, CRString* a_charset) {
    g_return_val_if_fail(a_this && a_this->type == AT_CHARSET_RULE_STMT && a_this->kind.charset_rule,
                         CR_BAD_PARAM_ERROR);
...
enum CRStatus cr_statement_at_font_face_rule_set_decls(CRStatement* a_this, CRDeclaration* a_decls) {
    g_return_val_if_fail(a_this && a_this->type == AT_FONT_FACE_RULE_STMT && a_this->kind.font_face_rule,
                         CR_BAD_PARAM_ERROR);
...
enum CRStatus cr_statement_at_font_face_rule_add_decl(CRStatement* a_this, CRString* a_prop, CRTerm* a_value) {
    ...
    decls = cr_declaration_append2(a_this->kind.font_face_rule->decl_list, a_prop, a_value);
...
gchar* cr_statement_to_string(CRStatement const* a_this, gulong a_indent) {
    ...
    switch (a_this->type) {
        ...
        case AT_FONT_FACE_RULE_STMT:
            str = cr_statement_font_face_rule_to_string(a_this, a_indent);
            break;

        case AT_CHARSET_RULE_STMT:
            str = cr_statement_charset_to_string(a_this, a_indent);
            break;
...
gchar* cr_statement_list_to_string(CRStatement const* a_this, gulong a_indent) {
    ...
    for (cur_stmt = a_this; cur_stmt; cur_stmt = cur_stmt->next) {
...
void cr_statement_dump_page(CRStatement const* a_this, FILE* a_fp, gulong a_indent) {
    ...
    g_return_if_fail(a_this && a_this->type == AT_PAGE_RULE_STMT && a_this->kind.page_rule);
...
void cr_statement_dump_import_rule(CRStatement const* a_this, FILE* a_fp, gulong a_indent) {
    gchar* str = NULL;
    g_return_if_fail(a_this && a_this->type == AT_IMPORT_RULE_STMT && a_fp && a_this->kind.import_rule);
