...
guchar* cr_attr_sel_to_string(CRAttrSel const* a_this) {
    ...
    for (cur = a_this; cur; cur = cur->next) {
        ...
        if (cur->name) {
            ...
            name = (guchar*)g_strndup(cur->name->stryng->str, cur->name->stryng->len);
            if (name) {
        ...
        if (cur->value) {
            ...
            value = (guchar*)g_strndup(cur->value->stryng->str, cur->value->stryng->len);
            if (value) {
                ...
                g_string_append_printf(str_buf, "\"%s\"", value);
