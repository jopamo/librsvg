...
enum CRStatus cr_utils_utf8_str_len_as_ucs4(const guchar* a_in_start, const guchar* a_in_end, gulong* a_len) {
    ...
    *a_len = 0;
    ...
    for (byte_ptr = (guchar*)a_in_start; byte_ptr <= a_in_end; byte_ptr++) {
...
enum CRStatus cr_utils_ucs4_str_len_as_utf8(const guint32* a_in_start, const guint32* a_in_end, gulong* a_len) {
    ...
    for (char_ptr = (guint32*)a_in_start; char_ptr <= a_in_end; char_ptr++) {
        if (*char_ptr <= 0x7F) {
...
enum CRStatus cr_utils_ucs1_str_len_as_utf8(const guchar* a_in_start, const guchar* a_in_end, gulong* a_len) {
    ...
    for (char_ptr = (guchar*)a_in_start; char_ptr <= a_in_end; char_ptr++) {
        if (*char_ptr <= 0x7F) {
            /*the utf-8 char would take 1 byte */
            len += 1;
        }
...
enum CRStatus cr_utils_utf8_to_ucs4(const guchar* a_in, gulong* a_in_len, guint32* a_out, gulong* a_out_len) {
    ...
    g_return_val_if_fail(a_in && a_in_len && a_out && a_out_len, CR_BAD_PARAM_ERROR);
    ...
    for (in_index = 0, out_index = 0; (in_index < in_len) && (out_index < out_len); in_index++, out_index++) {
        gint nb_bytes_2_decode = 0;
...
enum CRStatus cr_utils_read_char_from_utf8_buf(const guchar* a_in,
                                               ...
                                               guint32* a_out,
                                               gulong* a_consumed) {
    gulong in_index = 0, nb_bytes_2_decode = 0;
    enum CRStatus status = CR_OK;
    ...
    g_return_val_if_fail(a_in && a_out && a_out && a_consumed, CR_BAD_PARAM_ERROR);
...
enum CRStatus cr_utils_utf8_str_to_ucs4(const guchar* a_in, gulong* a_in_len, guint32** a_out, gulong* a_out_len) {
    ...
    status = cr_utils_utf8_str_len_as_ucs4(a_in, &a_in[*a_in_len - 1], a_out_len);
...
enum CRStatus cr_utils_ucs4_to_utf8(const guint32* a_in, gulong* a_in_len, guchar* a_out, gulong* a_out_len) {
    ...
    for (in_index = 0; in_index < in_len; in_index++) {
        ...
        if (a_in[in_index] <= 0x7F) {
            ...
            a_out[out_index + 1] = (0x80 | ((a_in[in_index] >> 6) & 0x3F));
            a_out[out_index + 2] = (0x80 | (a_in[in_index] & 0x3F));
            out_index += 3;
        }
        else if (a_in[in_index] <= 0x1FFFFF) {
            a_out[out_index] = (0xF0 | (a_in[in_index] >> 18));
            a_out[out_index + 1] = (0x80 | ((a_in[in_index] >> 12) & 0x3F));
            a_out[out_index + 2] = (0x80 | ((a_in[in_index] >> 6) & 0x3F));
            a_out[out_index + 3] = (0x80 | (a_in[in_index] & 0x3F));
            out_index += 4;
        }
        else if (a_in[in_index] <= 0x3FFFFFF) {
            a_out[out_index] = (0xF8 | (a_in[in_index] >> 24));
            a_out[out_index + 1] = (0x80 | (a_in[in_index] >> 18));
            a_out[out_index + 2] = (0x80 | ((a_in[in_index] >> 12) & 0x3F));
            a_out[out_index + 3] = (0x80 | ((a_in[in_index] >> 6) & 0x3F));
            a_out[out_index + 4] = (0x80 | (a_in[in_index] & 0x3F));
            out_index += 5;
        }
        else if (a_in[in_index] <= 0x7FFFFFFF) {
            a_out[out_index] = (0xFC | (a_in[in_index] >> 30));
            a_out[out_index + 1] = (0x80 | (a_in[in_index] >> 24));
            a_out[out_index + 2] = (0x80 | ((a_in[in_index] >> 18) & 0x3F));
            a_out[out_index + 3] = (0x80 | ((a_in[in_index] >> 12) & 0x3F));
            a_out[out_index + 4] = (0x80 | ((a_in[in_index] >> 6) & 0x3F));
            a_out[out_index + 4] = (0x80 | (a_in[in_index] & 0x3F));
            out_index += 6;
        }
        else {
...
enum CRStatus cr_utils_ucs4_str_to_utf8(const guint32* a_in, gulong* a_in_len, guchar** a_out, gulong* a_out_len) {
    ...
    status = cr_utils_ucs4_str_len_as_utf8(a_in, &a_in[*a_out_len - 1], a_out_len);
...
enum CRStatus cr_utils_ucs1_to_utf8(const guchar* a_in, gulong* a_in_len, guchar* a_out, gulong* a_out_len) {
    ...
    g_return_val_if_fail(a_in && a_in_len && a_out_len, CR_BAD_PARAM_ERROR);
    ...
    for (in_index = 0, out_index = 0; (in_index < in_len) && (out_index < out_len); in_index++) {
        /*
         *FIXME: return whenever we encounter forbidden char values.
         */
...
enum CRStatus cr_utils_ucs1_str_to_utf8(const guchar* a_in, gulong* a_in_len, guchar** a_out, gulong* a_out_len) {
    ...
    status = cr_utils_ucs1_str_len_as_utf8(a_in, &a_in[*a_in_len - 1], &out_len);
...
gboolean cr_utils_is_hexa_char(guint32 a_char) {
    if ((a_char >= '0' && a_char <= '9') || (a_char >= 'A' && a_char <= 'F')) {
        return TRUE;
    }
    return FALSE;
}
...
GList* cr_utils_dup_glist_of_string(GList const* a_list_of_strings) {
    ...
    for (cur = a_list_of_strings; cur; cur = cur->next) {
        ...
        str = g_string_new_len(((GString*)cur->data)->str, ((GString*)cur->data)->len);
        if (str)
