...
guchar* cr_simple_sel_to_string(CRSimpleSel const* a_this) {
    ...
    for (cur = a_this; cur; cur = cur->next) {
        if (cur->name) {
            guchar* str = (guchar*)g_strndup(cur->name->stryng->str, cur->name->stryng->len);
...
guchar* cr_simple_sel_one_to_string(CRSimpleSel const* a_this) {
    ...
    str_buf = g_string_new(NULL);
    if (a_this->name) {
        guchar* str = (guchar*)g_strndup(a_this->name->stryng->str, a_this->name->stryng->len);
...
enum CRStatus cr_simple_sel_compute_specificity(CRSimpleSel* a_this) {
    ...
    for (cur_sel = a_this; cur_sel; cur_sel = cur_sel->next) {
        ...
        else if (!cur_sel->name || !cur_sel->name->stryng || !cur_sel->name->stryng->str) {
        ...
        for (cur_add_sel = cur_sel->add_sel; cur_add_sel; cur_add_sel = cur_add_sel->next) {
