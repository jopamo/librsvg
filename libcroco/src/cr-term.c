...
guchar* cr_term_to_string(CRTerm const* a_this) {
    ...
    for (cur = a_this; cur; cur = cur->next) {
        if ((cur->content.str == NULL) && (cur->content.num == NULL) && (cur->content.str == NULL) &&
        ...
        switch (cur->type) {
            ...
            case TERM_FUNCTION:
                if (cur->content.str) {
                    content = (guchar*)g_strndup(cur->content.str->stryng->str, cur->content.str->stryng->len);
                }

                if (content) {
                    g_string_append_printf(str_buf, "%s(", content);

                    if (cur->ext_content.func_param) {
                        guchar* tmp_str = NULL;

                        tmp_str = cr_term_to_string(cur->ext_content.func_param);
            ...
            case TERM_URI:
                ...
                if (content) {
                    g_string_append_printf(str_buf, "url(%s)", content);
                    g_free(content);
                    content = NULL;
                }
                break;
            ...
            case TERM_UNICODERANGE:
                g_string_append(str_buf, "?found unicoderange: dump not supported yet?");
                break;
            ...
            default:
                g_string_append(str_buf, "Unrecognized Term type");
                break;
        }
    }
...
guchar* cr_term_one_to_string(CRTerm const* a_this) {
    ...
    if ((a_this->content.str == NULL) && (a_this->content.num == NULL) && (a_this->content.str == NULL) &&
        (a_this->content.rgb == NULL))
        return NULL;
    ...
    switch (a_this->type) {
        ...
        case TERM_FUNCTION:
            if (a_this->content.str) {
                content = (guchar*)g_strndup(a_this->content.str->stryng->str, a_this->content.str->stryng->len);
            }
            ...
            if (content) {
                ...
                if (a_this->ext_content.func_param) {
                    ...
                    tmp_str = cr_term_to_string(a_this->ext_content.func_param);

                    if (tmp_str) {
                        g_string_append_printf(str_buf, "%s", tmp_str);
                        g_free(tmp_str);
                        tmp_str = NULL;
                    }
        ...
        case TERM_UNICODERANGE:
            g_string_append_printf(str_buf, "?found unicoderange: dump not supported yet?");
            break;
        ...
        default:
            g_string_append_printf(str_buf, "%s", "Unrecognized Term type");
            break;
    }
