...
static enum CRStatus cr_font_family_to_string_real(CRFontFamily const* a_this,
    ...
    if (!*a_string) {
        *a_string = g_string_new(NULL);
        g_return_val_if_fail(*a_string, CR_INSTANCIATION_FAILED_ERROR);
    }
    ...
    if (a_walk_list == TRUE && a_this->next) {
        result = cr_font_family_to_string_real(a_this->next, TRUE, a_string);
    }
    return result;
}

static const gchar* cr_predefined_absolute_font_size_to_string(enum CRPredefinedAbsoluteFontSize a_code) {
...
static const gchar* cr_relative_font_size_to_string(enum CRRelativeFontSize a_code) {
    gchar const* str = NULL;
...
enum CRStatus cr_font_size_copy(CRFontSize* a_dst, CRFontSize const* a_src) {
    ...
    switch (a_src->type) {
        ...
        case ABSOLUTE_FONT_SIZE:
            cr_font_size_clear(a_dst);
            cr_num_copy(&a_dst->value.absolute, &a_src->value.absolute);
...
enum CRStatus cr_font_size_set_predefined_absolute_font_size(CRFontSize* a_this,
    ...
    g_return_val_if_fail(a_this, CR_BAD_PARAM_ERROR);
    g_return_val_if_fail(a_predefined >= FONT_SIZE_XX_SMALL && a_predefined < NB_PREDEFINED_ABSOLUTE_FONT_SIZES,
                         CR_BAD_PARAM_ERROR);
...
enum CRStatus cr_font_size_set_relative_font_size(CRFontSize* a_this, enum CRRelativeFontSize a_relative) {
    g_return_val_if_fail(a_this, CR_BAD_PARAM_ERROR);
    g_return_val_if_fail(a_relative >= FONT_SIZE_LARGER && a_relative < NB_RELATIVE_FONT_SIZE, CR_BAD_PARAM_ERROR);
...
enum CRStatus cr_font_size_set_absolute_font_size(CRFontSize* a_this, enum CRNumType a_num_type, gdouble a_value) {
    g_return_val_if_fail(a_this, CR_BAD_PARAM_ERROR);
    g_return_val_if_fail(a_num_type >= NUM_AUTO && a_num_type < NB_NUM_TYPE, CR_BAD_PARAM_ERROR);

    a_this->type = ABSOLUTE_FONT_SIZE;
    cr_num_set(&a_this->value.absolute, a_value, a_num_type);
    return CR_OK;
}
...
gchar* cr_font_size_to_string(CRFontSize const* a_this) {
    ...
    switch (a_this->type) {
        case PREDEFINED_ABSOLUTE_FONT_SIZE:
            str = g_strdup(cr_predefined_absolute_font_size_to_string(a_this->value.predefined));
            break;
        case ABSOLUTE_FONT_SIZE:
            str = (gchar*)cr_num_to_string(&a_this->value.absolute);
            break;
        case RELATIVE_FONT_SIZE:
            str = g_strdup(cr_relative_font_size_to_string(a_this->value.relative));
            break;
        case INHERITED_FONT_SIZE:
...
void cr_font_size_get_smaller_predefined_font_size(enum CRPredefinedAbsoluteFontSize a_font_size,
    ...
    g_return_if_fail(a_font_size < NB_PREDEFINED_ABSOLUTE_FONT_SIZES && a_font_size >= FONT_SIZE_XX_SMALL);
...
void cr_font_size_get_larger_predefined_font_size(enum CRPredefinedAbsoluteFontSize a_font_size,
    ...
    g_return_if_fail(a_font_size >= FONT_SIZE_XX_SMALL && a_font_size < NB_PREDEFINED_ABSOLUTE_FONT_SIZES);
...
gboolean cr_font_size_is_predefined_absolute_font_size(enum CRPredefinedAbsoluteFontSize a_font_size) {
    if (a_font_size >= FONT_SIZE_XX_SMALL && a_font_size < NB_PREDEFINED_ABSOLUTE_FONT_SIZES) {
        return TRUE;
    }
    else {
        return FALSE;
...
enum CRFontWeight cr_font_weight_get_bolder(enum CRFontWeight a_weight) {
    ...
    else if (a_weight == FONT_WEIGHT_BOLDER || a_weight == FONT_WEIGHT_LIGHTER) {
