...
static enum CRStatus cr_om_parser_init_default_sac_handler(CROMParser* a_this) {
    ...
    g_return_val_if_fail(a_this && PRIVATE(a_this) && PRIVATE(a_this)->parser, CR_BAD_PARAM_ERROR);

    status = cr_parser_get_sac_handler(PRIVATE(a_this)->parser, &sac_handler);
    ...
    if (created_handler) {
        status = cr_parser_set_sac_handler(PRIVATE(a_this)->parser, sac_handler);
...
static void start_font_face(CRDocHandler* a_this, CRParsingLocation* a_location) {
    ...
    ctxt->cur_stmt = cr_statement_new_at_font_face_rule(ctxt->stylesheet, NULL);
...
static void end_font_face(CRDocHandler* a_this) {
    ...
    status = cr_doc_handler_get_ctxt(a_this, (gpointer*)ctxtptr);
    g_return_if_fail(status == CR_OK && ctxt);
    g_return_if_fail(ctxt->cur_stmt && ctxt->cur_stmt->type == AT_FONT_FACE_RULE_STMT && ctxt->stylesheet);

    stmts = cr_statement_append(ctxt->stylesheet->statements, ctxt->cur_stmt);
    if (!stmts)
        goto error;
...
static void start_page(CRDocHandler* a_this, CRString* a_page, CRString* a_pseudo, CRParsingLocation* a_location) {
    enum CRStatus status = CR_OK;
    ParsingContext* ctxt = NULL;
    ParsingContext** ctxtptr = NULL;
    ...
    ctxt->cur_stmt = cr_statement_new_at_page_rule(ctxt->stylesheet, NULL, NULL, NULL);
    if (a_page) {
        ctxt->cur_stmt->kind.page_rule->name = cr_string_dup(a_page);
    ...
    if (a_pseudo) {
        ctxt->cur_stmt->kind.page_rule->pseudo = cr_string_dup(a_pseudo);
...
static void end_page(CRDocHandler* a_this, CRString* a_page, CRString* a_pseudo_page) {
    ...
    g_return_if_fail(ctxt->cur_stmt && ctxt->cur_stmt->type == AT_PAGE_RULE_STMT && ctxt->stylesheet);

    stmt = cr_statement_append(ctxt->stylesheet->statements, ctxt->cur_stmt);
...
static void start_media(CRDocHandler* a_this, GList* a_media_list, CRParsingLocation* a_location) {
    ...
    g_return_if_fail(ctxt && ctxt->cur_stmt == NULL && ctxt->cur_media_stmt == NULL && ctxt->stylesheet);
    if (a_media_list) {
...
static void end_media(CRDocHandler* a_this, GList* a_media_list) {
    ...
    g_return_if_fail(ctxt && ctxt->cur_media_stmt && ctxt->cur_media_stmt->type == AT_MEDIA_RULE_STMT &&
                     ctxt->stylesheet);

    stmts = cr_statement_append(ctxt->stylesheet->statements, ctxt->cur_media_stmt);
...
static void import_style(CRDocHandler* a_this,
    ...
    stmt = cr_statement_new_at_import_rule(ctxt->stylesheet, uri, media_list, NULL);
...
static void start_selector(CRDocHandler* a_this, CRSelector* a_selector_list) {
    ...
    ctxt->cur_stmt = cr_statement_new_ruleset(ctxt->stylesheet, a_selector_list, NULL, NULL);
}
...
static void end_selector(CRDocHandler* a_this, CRSelector* a_selector_list) {
    ...
    if (ctxt->cur_stmt) {
        ...
        if (ctxt->cur_media_stmt) {
            ...
            stmts = cr_statement_append(media_rule->rulesets, ctxt->cur_stmt);

            if (!stmts) {
                cr_utils_trace_info("Could not append a new statement");
                cr_statement_destroy(media_rule->rulesets);
                ctxt->cur_media_stmt->kind.media_rule->rulesets = NULL;
                return;
            }
...
static void property(CRDocHandler* a_this, CRString* a_name, CRTerm* a_expression, gboolean a_important) {
     ...
     */
    g_return_if_fail(ctxt->cur_stmt &&
                     (ctxt->cur_stmt->type == RULESET_STMT || ctxt->cur_stmt->type == AT_FONT_FACE_RULE_STMT ||
    ...
    switch (ctxt->cur_stmt->type) {
        case RULESET_STMT:
            decl2 = cr_declaration_append(ctxt->cur_stmt->kind.ruleset->decl_list, decl);
            if (!decl2) {
                cr_declaration_destroy(decl);
                cr_utils_trace_info("Could not append decl to ruleset");
                goto error;
            }
        ...
        case AT_FONT_FACE_RULE_STMT:
            decl2 = cr_declaration_append(ctxt->cur_stmt->kind.font_face_rule->decl_list, decl);
            if (!decl2) {
                cr_declaration_destroy(decl);
                cr_utils_trace_info("Could not append decl to ruleset");
                goto error;
        ...
        case AT_PAGE_RULE_STMT:
            decl2 = cr_declaration_append(ctxt->cur_stmt->kind.page_rule->decl_list, decl);
            if (!decl2) {
                cr_declaration_destroy(decl);
                cr_utils_trace_info("Could not append decl to ruleset");
                goto error;
...
enum CRStatus cr_om_parser_parse_buf(CROMParser* a_this,
    ...
    status = cr_parser_parse_buf(PRIVATE(a_this)->parser, a_buf, a_len, a_enc);
    ...
    if (status == CR_OK) {
        ...
        cr_parser_get_sac_handler(PRIVATE(a_this)->parser, &sac_handler);
...
enum CRStatus cr_om_parser_simply_parse_buf(const guchar* a_buf,
    ...
    status = cr_om_parser_parse_buf(parser, a_buf, a_len, a_enc, a_result);
...
enum CRStatus cr_om_parser_parse_file(CROMParser* a_this,
    ...
    g_return_val_if_fail(a_this && a_file_uri && a_result, CR_BAD_PARAM_ERROR);

    if (!PRIVATE(a_this)->parser) {
        PRIVATE(a_this)->parser = cr_parser_new_from_file(a_file_uri, a_enc);
    }

    status = cr_parser_parse_file(PRIVATE(a_this)->parser, a_file_uri, a_enc);
...
enum CRStatus cr_om_parser_simply_parse_file(const guchar* a_file_path,
    ...
    status = cr_om_parser_parse_file(parser, a_file_path, a_enc, a_result);
    if (parser) {
        cr_om_parser_destroy(parser);
        parser = NULL;
    }
...
enum CRStatus cr_om_parser_parse_paths_to_cascade(CROMParser* a_this,
    ...
    for (i = 0; i < 3; i++) {
        status = cr_om_parser_parse_file(a_this, paths[i], a_encoding, &sheets[i]);
        if (status != CR_OK) {
            if (sheets[i]) {
...
enum CRStatus cr_om_parser_simply_parse_paths_to_cascade(const guchar* a_author_path,
    ...
    status = cr_om_parser_parse_paths_to_cascade(parser, a_author_path, a_user_path, a_ua_path, a_encoding, a_result);
    if (parser) {
        cr_om_parser_destroy(parser);
        parser = NULL;
    }
