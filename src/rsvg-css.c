...
typedef enum _RelativeSize { RELATIVE_SIZE_NORMAL, RELATIVE_SIZE_SMALLER, RELATIVE_SIZE_LARGER } RelativeSize;

static double rsvg_css_parse_raw_length(const char* str,
                                        gboolean* in,
                                        gboolean* percent,
                                        gboolean* em,
                                        gboolean* ex,
...
double _rsvg_css_normalize_length(const RsvgLength* in, RsvgDrawingCtx* ctx, char dir) {
    if (in->factor == '\0')
        ...
        if (dir == 'o')
            return in->length * rsvg_viewport_percentage(ctx->vb.rect.width, ctx->vb.rect.height);
    }
...
static gint rsvg_css_clip_rgb_percent(const char* s, double max) {
    double value;
    char* end;
...
guint rsvg_css_parse_opacity(const char* str) {
    ...
    if (((opacity == -HUGE_VAL || opacity == HUGE_VAL) && (ERANGE == errno)) || *end_ptr != '\0')
        opacity = 1.;
...
char** rsvg_css_parse_xml_attribute_string(const char* attribute_string) {
    ...
    if ((doc = parser->myDoc) == NULL || (node = doc->children) == NULL ||
        strcmp((const char*)node->name, "rsvg-hack") != 0 || node->next != NULL || node->properties == NULL)
        goto done;
...
void rsvg_parse_cssbuffer(RsvgHandle* ctx, const char* buff, size_t buflen) {
    rsvg_css_engine_parse_stylesheet(ctx->priv->css_engine, buff, buflen);
}
