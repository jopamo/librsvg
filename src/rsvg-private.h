...
typedef enum { RSVG_LOAD_POLICY_STRICT } RsvgLoadPolicy;
...
struct RsvgRender {
    ...
    void (*render_surface)(RsvgDrawingCtx* ctx, cairo_surface_t* surface, double x, double y, double w, double h);
    ...
    cairo_surface_t* (*get_surface_of_node)(RsvgDrawingCtx* ctx, RsvgNode* drawable, double w, double h);
};

static inline RsvgRender* _rsvg_render_check_type(RsvgRender* render, RsvgRenderType type) {
...
typedef enum { objectBoundingBox, userSpaceOnUse } RsvgCoordUnits;
...
G_GNUC_INTERNAL
double _rsvg_css_hand_normalize_length(const RsvgLength* in,
                                       gdouble pixels_per_inch,
                                       gdouble width_or_height,
                                       gdouble font_size);
...
G_GNUC_INTERNAL
char* _rsvg_handle_acquire_data(RsvgHandle* handle, const char* uri, char** content_type, gsize* len, GError** error);
G_GNUC_INTERNAL
GInputStream* _rsvg_handle_acquire_stream(RsvgHandle* handle, const char* uri, char** content_type, GError** error);

G_GNUC_INTERNAL
xmlParserCtxtPtr rsvg_free_xml_parser_and_doc(xmlParserCtxtPtr ctxt) G_GNUC_WARN_UNUSED_RESULT;

#define rsvg_return_if_fail(expr, error)                          \
    G_STMT_START {                                                \
        if G_LIKELY (expr) {                                      \
        }                                                         \
        else {                                                    \
            rsvg_return_if_fail_warning(G_STRFUNC, #expr, error); \
            return;                                               \
        };                                                        \
    }                                                             \
    G_STMT_END

#define rsvg_return_val_if_fail(expr, val, error)                 \
    G_STMT_START {                                                \
        if G_LIKELY (expr) {                                      \
        }                                                         \
        else {                                                    \
