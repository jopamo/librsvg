...
static void rsvg_node_path_free(RsvgNode* self) {
...
static void rsvg_node_path_draw(RsvgNode* self, RsvgDrawingCtx* ctx, int dominate) {
    RsvgNodePath* path = (RsvgNodePath*)self;
...
static void rsvg_node_path_set_atts(RsvgNode* self, RsvgHandle* ctx, RsvgPropertyBag* atts) {
...
static cairo_path_t* _rsvg_node_poly_build_path(const char* value, gboolean close_path);

static void _rsvg_node_poly_set_atts(RsvgNode* self, RsvgHandle* ctx, RsvgPropertyBag* atts) {
    ...
    if (rsvg_property_bag_size(atts)) {
        ...
        if ((value = rsvg_property_bag_lookup(atts, "verts")) || (value = rsvg_property_bag_lookup(atts, "points"))) {
            ...
            poly->path = _rsvg_node_poly_build_path(value, RSVG_NODE_TYPE(self) == RSVG_NODE_TYPE_POLYGON);
        }
        if ((value = rsvg_property_bag_lookup(atts, "class")))
            klazz = value;
        if ((value = rsvg_property_bag_lookup(atts, "id"))) {
        ...
        rsvg_parse_style_attrs(ctx, self->state,
                               RSVG_NODE_TYPE(self) == RSVG_NODE_TYPE_POLYLINE ? "polyline" : "polygon", klazz, id,
                               atts);
    }
}
...
static void _rsvg_node_poly_draw(RsvgNode* self, RsvgDrawingCtx* ctx, int dominate) {
    RsvgNodePoly* poly = (RsvgNodePoly*)self;
...
static void _rsvg_node_poly_free(RsvgNode* self) {
    RsvgNodePoly* poly = (RsvgNodePoly*)self;
    if (poly->path)
        rsvg_cairo_path_destroy(poly->path);
...
static RsvgNode* rsvg_new_any_poly(RsvgNodeType type) {
    RsvgNodePoly* poly;
...
static void _rsvg_node_line_set_atts(RsvgNode* self, RsvgHandle* ctx, RsvgPropertyBag* atts) {
    const char *klazz = NULL, *id = NULL, *value;
...
static void _rsvg_node_line_draw(RsvgNode* overself, RsvgDrawingCtx* ctx, int dominate) {
    cairo_path_t* path;
    RsvgPathBuilder builder;
...
static void _rsvg_node_rect_set_atts(RsvgNode* self, RsvgHandle* ctx, RsvgPropertyBag* atts) {
    const char *klazz = NULL, *id = NULL, *value;
...
static void _rsvg_node_rect_draw(RsvgNode* self, RsvgDrawingCtx* ctx, int dominate) {
    ...
    if (rx == 0) {
        ...
        rsvg_path_builder_arc(&builder, top_x2, top_y, rx, ry, 0, FALSE, TRUE, right_x, right_y1);

        rsvg_path_builder_line_to(&builder, right_x, right_y2);

        rsvg_path_builder_arc(&builder, right_x, right_y2, rx, ry, 0, FALSE, TRUE, bottom_x2, bottom_y);

        rsvg_path_builder_line_to(&builder, bottom_x1, bottom_y);

        rsvg_path_builder_arc(&builder, bottom_x1, bottom_y, rx, ry, 0, FALSE, TRUE, left_x, left_y2);

        rsvg_path_builder_line_to(&builder, left_x, left_y1);

        rsvg_path_builder_arc(&builder, left_x, left_y1, rx, ry, 0, FALSE, TRUE, top_x1, top_y);
...
static void _rsvg_node_circle_set_atts(RsvgNode* self, RsvgHandle* ctx, RsvgPropertyBag* atts) {
    const char *klazz = NULL, *id = NULL, *value;
...
static void _rsvg_node_circle_draw(RsvgNode* self, RsvgDrawingCtx* ctx, int dominate) {
    cairo_path_t* path;
    RsvgNodeCircle* circle = (RsvgNodeCircle*)self;
    double cx, cy, r;
    RsvgPathBuilder builder;
    ...
    rsvg_path_builder_curve_to(&builder, cx + r, cy + r * RSVG_ARC_MAGIC, cx + r * RSVG_ARC_MAGIC, cy + r, cx, cy + r);

    rsvg_path_builder_curve_to(&builder, cx - r * RSVG_ARC_MAGIC, cy + r, cx - r, cy + r * RSVG_ARC_MAGIC, cx - r, cy);
...
static void _rsvg_node_ellipse_set_atts(RsvgNode* self, RsvgHandle* ctx, RsvgPropertyBag* atts) {
    const char *klazz = NULL, *id = NULL, *value;
...
static void _rsvg_node_ellipse_draw(RsvgNode* self, RsvgDrawingCtx* ctx, int dominate) {
    RsvgNodeEllipse* ellipse = (RsvgNodeEllipse*)self;
    cairo_path_t* path;
    double cx, cy, rx, ry;
    RsvgPathBuilder builder;
    ...
    rsvg_path_builder_curve_to(&builder, cx + rx, cy - RSVG_ARC_MAGIC * ry, cx + RSVG_ARC_MAGIC * rx, cy - ry, cx,
                               cy - ry);

    rsvg_path_builder_curve_to(&builder, cx - RSVG_ARC_MAGIC * rx, cy - ry, cx - rx, cy - RSVG_ARC_MAGIC * ry, cx - rx,
                               cy);
