...
static void rsvg_node_group_set_atts(RsvgNode* self, RsvgHandle* ctx, RsvgPropertyBag* atts) {
...
static void rsvg_node_use_draw(RsvgNode* self, RsvgDrawingCtx* ctx, int dominate) {
    ...
    if (RSVG_NODE_TYPE(child) != RSVG_NODE_TYPE_SYMBOL) {
        ...
        if (symbol->vbox.active) {
            rsvg_preserve_aspect_ratio(symbol->preserve_aspect_ratio, symbol->vbox.rect.width, symbol->vbox.rect.height,
                                       &w, &h, &x, &y);
            ...
            if (!state->overflow || (!state->has_overflow && child->state->overflow))
                rsvg_add_clipping_rect(ctx, symbol->vbox.rect.x, symbol->vbox.rect.y, symbol->vbox.rect.width,
...
static void rsvg_node_svg_draw(RsvgNode* self, RsvgDrawingCtx* ctx, int dominate) {
    RsvgNodeSvg* sself;
    RsvgState* state;
    ...
    if (sself->vbox.active) {
        ...
        rsvg_preserve_aspect_ratio(sself->preserve_aspect_ratio, sself->vbox.rect.width, sself->vbox.rect.height, &w,
                                   &h, &x, &y);
        cairo_matrix_init(&affine, w / sself->vbox.rect.width, 0, 0, h / sself->vbox.rect.height,
...
static void rsvg_node_svg_set_atts(RsvgNode* self, RsvgHandle* ctx, RsvgPropertyBag* atts) {
    const char* value;
...
static void rsvg_node_use_free(RsvgNode* node) {
    RsvgNodeUse* use = (RsvgNodeUse*)node;
    g_free(use->link);
    _rsvg_node_free(node);
}

static void rsvg_node_use_set_atts(RsvgNode* self, RsvgHandle* ctx, RsvgPropertyBag* atts) {
...
static void rsvg_node_symbol_set_atts(RsvgNode* self, RsvgHandle* ctx, RsvgPropertyBag* atts) {
...
static void _rsvg_node_switch_draw(RsvgNode* self, RsvgDrawingCtx* ctx, int dominate) {
    guint i;
