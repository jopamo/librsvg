...
static void _pattern_add_rsvg_color_stops(cairo_pattern_t* pattern,
                                          GPtrArray* stops,
    ...
    for (i = 0; i < stops->len; i++) {
        ...
        rgba = stop->rgba;
        cairo_pattern_add_color_stop_rgba(pattern, stop->offset, ((rgba >> 24) & 0xff) / 255.0,
                                          ((rgba >> 16) & 0xff) / 255.0, ((rgba >> 8) & 0xff) / 255.0,
                                          (((rgba >> 0) & 0xff) * opacity) / 255.0 / 255.0);
    }
}

static void _set_source_rsvg_linear_gradient(RsvgDrawingCtx* ctx,
    ...
    pattern = cairo_pattern_create_linear(
        _rsvg_css_normalize_length(&linear->x1, ctx, 'h'), _rsvg_css_normalize_length(&linear->y1, ctx, 'v'),
        _rsvg_css_normalize_length(&linear->x2, ctx, 'h'), _rsvg_css_normalize_length(&linear->y2, ctx, 'v'));
    ...
    if (linear->obj_bbox) {
        ...
        cairo_matrix_init(&bboxmatrix, bbox.rect.width, 0, 0, bbox.rect.height, bbox.rect.x, bbox.rect.y);
...
static void _set_source_rsvg_radial_gradient(RsvgDrawingCtx* ctx,
    ...
    pattern = cairo_pattern_create_radial(
        _rsvg_css_normalize_length(&radial->fx, ctx, 'h'), _rsvg_css_normalize_length(&radial->fy, ctx, 'v'), 0.0,
        _rsvg_css_normalize_length(&radial->cx, ctx, 'h'), _rsvg_css_normalize_length(&radial->cy, ctx, 'v'),
...
static void _set_source_rsvg_solid_color(RsvgDrawingCtx* ctx,
...
static void _set_source_rsvg_pattern(RsvgDrawingCtx* ctx, RsvgPattern* rsvg_pattern, guint8 opacity, RsvgBbox bbox) {
    RsvgCairoRender* render = RSVG_CAIRO_RENDER(ctx->render);
    ...
    surface = cairo_surface_create_similar(cairo_get_target(cr_render), CAIRO_CONTENT_COLOR_ALPHA, pw, ph);
    cr_pattern = cairo_create(surface);

    /* Create the pattern coordinate system */
    if (rsvg_pattern->obj_bbox) {
        /* subtract the pattern origin */
        cairo_matrix_init_translate(&affine, bbox.rect.x + patternx * bbox.rect.width,
    ...
    if (rsvg_pattern->vbox.active) {
        ...
        x = 0;
        y = 0;
        rsvg_preserve_aspect_ratio(rsvg_pattern->preserve_aspect_ratio, rsvg_pattern->vbox.rect.width,
                                   rsvg_pattern->vbox.rect.height, &w, &h, &x, &y);

        x -= rsvg_pattern->vbox.rect.x * w / rsvg_pattern->vbox.rect.width;
        y -= rsvg_pattern->vbox.rect.y * h / rsvg_pattern->vbox.rect.height;

        cairo_matrix_init(&caffine, w / rsvg_pattern->vbox.rect.width, 0, 0, h / rsvg_pattern->vbox.rect.height, x, y);
    ...
    rsvg_state_push(ctx);
    rsvg_current_state(ctx)->personal_affine = rsvg_current_state(ctx)->affine = caffine;
...
static void _set_source_rsvg_paint_server(RsvgDrawingCtx* ctx,
                                          guint32 current_color_rgb,
                                          RsvgPaintServer* ps,
                                          guint8 opacity,
                                          RsvgBbox bbox,
...
static void _set_rsvg_affine(RsvgCairoRender* render, cairo_matrix_t* affine) {
    cairo_t* cr = render->cr;
    cairo_matrix_t matrix;
    gboolean nest = cr != render->initial_cr;

    cairo_matrix_init(&matrix, affine->xx, affine->yx, affine->xy, affine->yy,
                      affine->x0 + (nest ? 0 : render->offset_x), affine->y0 + (nest ? 0 : render->offset_y));
    cairo_set_matrix(cr, &matrix);
}

#ifdef HAVE_PANGOFT2
static cairo_font_options_t* get_font_options_for_testing(void) {
...
static void set_font_options_for_testing(PangoContext* context) {
...
static void create_font_config_for_testing(RsvgCairoRender* render) {
...
static PangoFontMap* get_font_map_for_testing(RsvgCairoRender* render) {
    ...
    if (!render->font_map_for_testing) {
        ...
        pango_fc_font_map_set_config(PANGO_FC_FONT_MAP(render->font_map_for_testing), render->font_config_for_testing);
    }
...
void rsvg_cairo_render_pango_layout(RsvgDrawingCtx* ctx, PangoLayout* layout, double x, double y) {
    RsvgCairoRender* render = RSVG_CAIRO_RENDER(ctx->render);
    ...
    if (state->fill) {
        ...
        _set_source_rsvg_paint_server(ctx, state->current_color, state->fill, state->fill_opacity, bbox,
    ...
    if (state->stroke) {
        ...
        _set_source_rsvg_paint_server(ctx, state->current_color, state->stroke, state->stroke_opacity, bbox,
                                      rsvg_current_state(ctx)->current_color);
...
void rsvg_cairo_render_path(RsvgDrawingCtx* ctx, const cairo_path_t* path) {
    RsvgCairoRender* render = RSVG_CAIRO_RENDER(ctx->render);
    RsvgState* state = rsvg_current_state(ctx);
    cairo_t* cr;
    RsvgBbox bbox;
    ...
    cairo_set_dash(cr, state->dash.dash, state->dash.n_dash, _rsvg_css_normalize_length(&state->dash.offset, ctx, 'o'));
    ...
    if (state->fill != NULL) {
        ...
        _set_source_rsvg_paint_server(ctx, state->current_color, state->fill, opacity, bbox,
                                      rsvg_current_state(ctx)->current_color);
    ...
    if (state->stroke != NULL) {
        ...
        _set_source_rsvg_paint_server(ctx, state->current_color, state->stroke, opacity, bbox,
                                      rsvg_current_state(ctx)->current_color);

        cairo_stroke(cr);
    }

    cairo_new_path(cr); /* clear the path in case stroke == fill == NULL; otherwise we leave it around from computing
                           the bounding box */

    rsvg_cairo_pop_discrete_layer(ctx);
}

void rsvg_cairo_render_surface(RsvgDrawingCtx* ctx,
                               cairo_surface_t* surface,
                               double src_x,
                               double src_y,
                               double w,
...
static void rsvg_cairo_generate_mask(cairo_t* cr, RsvgMask* self, RsvgDrawingCtx* ctx, RsvgBbox* bbox) {
    RsvgCairoRender* render = RSVG_CAIRO_RENDER(ctx->render);
    cairo_surface_t* surface;
    ...
    if (self->maskunits == objectBoundingBox)
        rsvg_cairo_add_clipping_rect(ctx, sx * bbox->rect.width + bbox->rect.x, sy * bbox->rect.height + bbox->rect.y,
                                     sw * bbox->rect.width, sh * bbox->rect.height);
    else
    ...
    if (self->contentunits == objectBoundingBox) {
        ...
        cairo_matrix_init(&bbtransform, bbox->rect.width, 0, 0, bbox->rect.height, bbox->rect.x, bbox->rect.y);
    ...
    for (row = 0; row < height; row++) {
        ...
        for (i = 0; i < width; i++) {
             ...
             */
            *pixel = ((((*pixel & 0x00ff0000) >> 16) * 14042 + ((*pixel & 0x0000ff00) >> 8) * 47240 +
                       ((*pixel & 0x000000ff)) * 4769) *
                      state->opacity);
        }
    }
    ...
    cairo_mask_surface(cr, surface, nest ? 0 : render->offset_x, nest ? 0 : render->offset_y);
    cairo_surface_destroy(surface);
}

static void rsvg_cairo_push_render_stack(RsvgDrawingCtx* ctx) {
    RsvgCairoRender* render = RSVG_CAIRO_RENDER(ctx->render);
    cairo_surface_t* surface;
    cairo_t* child_cr;
    RsvgBbox* bbox;
    ...
    if (state->opacity == 0xFF && !state->filter && !state->mask && !lateclip &&
        (state->comp_op == CAIRO_OPERATOR_OVER) && (state->enable_background == RSVG_ENABLE_BACKGROUND_ACCUMULATE))
        return;

    if (!state->filter) {
        surface = cairo_surface_create_similar(cairo_get_target(render->cr), CAIRO_CONTENT_COLOR_ALPHA, render->width,
                                               render->height);
    }
    else {
...
static void rsvg_cairo_pop_render_stack(RsvgDrawingCtx* ctx) {
    RsvgCairoRender* render = RSVG_CAIRO_RENDER(ctx->render);
    ...
    cairo_set_source_surface(render->cr, surface, nest ? 0 : render->offset_x, nest ? 0 : render->offset_y);
...
void rsvg_cairo_add_clipping_rect(RsvgDrawingCtx* ctx, double x, double y, double w, double h) {
    RsvgCairoRender* render = RSVG_CAIRO_RENDER(ctx->render);
    cairo_t* cr = render->cr;
...
cairo_surface_t* rsvg_cairo_get_surface_of_node(RsvgDrawingCtx* ctx, RsvgNode* drawable, double width, double height) {
    cairo_surface_t* surface;
    cairo_t* cr;
...
static void convert_no_alpha(guchar* dest_data,
                             int dest_stride,
                             guchar* src_data,
                             int src_stride,
                             int src_x,
