...
void rsvg_path_builder_init(RsvgPathBuilder* builder, int n_elements) {
    builder->path_data = g_array_sized_new(FALSE, FALSE, sizeof(cairo_path_data_t), n_elements);
    builder->last_move_to_index = -1;
}

void rsvg_path_builder_move_to(RsvgPathBuilder* builder, double x, double y) {
...
void rsvg_path_builder_line_to(RsvgPathBuilder* builder, double x, double y) {
    cairo_path_data_t data;
...
static void rsvg_path_arc_segment(RsvgPathBuilder* builder,
                                  double xc,
                                  double yc,
                                  double th0,
                                  double th1,
    ...
    rsvg_path_builder_curve_to(builder, xc + cosf * x1 - sinf * y1, yc + sinf * x1 + cosf * y1,
                               xc + cosf * x2 - sinf * y2, yc + sinf * x2 + cosf * y2, xc + cosf * x3 - sinf * y3,
                               yc + sinf * x3 + cosf * y3);
}
...
void rsvg_path_builder_arc(RsvgPathBuilder* builder,
    ...
    for (i = 0; i < n_segs; i++)
        rsvg_path_arc_segment(builder, cx, cy, theta1 + i * delta_theta / n_segs,
                              theta1 + (i + 1) * delta_theta / n_segs, rx, ry, x_axis_rotation);
}
    ...
    switch (ctx->cmd) {
        ...
        case 'a':
            if (ctx->param == 7 || final) {
                ...
                rsvg_path_builder_arc(&ctx->builder, x1, y1, rx, ry, x_axis_rotation, large_arc_flag, sweep_flag, x2,
                                      y2);
...
static void rsvg_path_end_of_number(RSVGParsePathCtx* ctx, double val, int sign, int exp_sign, int exp) {
 ...
 */
static int rsvg_parse_number(RSVGParsePathCtx* ctx, const char* data) {
    int length = 0;
...
static void rsvg_parse_path_data(RSVGParsePathCtx* ctx, const char* data) {
    ...
    for (i = 0; data[i] != '\0'; i++) {
        ...
        if ((c >= '0' && c <= '9') || c == '+' || c == '-' || c == '.') {
            ...
            ctx->cp = ctx->rp =
                g_array_index(ctx->builder.path_data, cairo_path_data_t, ctx->builder.path_data->len - 1);
        }
        else if (c >= 'A' && c < 'Z' && c != 'E') {
            if (ctx->param)
                rsvg_parse_path_do_cmd(ctx, TRUE);
