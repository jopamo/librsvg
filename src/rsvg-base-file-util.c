...
static gboolean rsvg_handle_fill_with_data(RsvgHandle* handle, const char* data, gsize data_len, GError** error) {
    gboolean rv;
...
RsvgHandle* rsvg_handle_new_from_file(const gchar* file_name, GError** error) {
    ...
    if (!base_uri) {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Cannot obtain URI from '%s'"), file_name);
        g_object_unref(file);
        return NULL;
    }
