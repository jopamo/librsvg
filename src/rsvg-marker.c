...
static void rsvg_node_marker_set_atts(RsvgNode* self, RsvgHandle* ctx, RsvgPropertyBag* atts) {
...
static void rsvg_marker_render(const char* marker_name,
                               gdouble xpos,
                               gdouble ypos,
                               gdouble orient,
                               gdouble linewidth,
    ...
    if (self->vbox.active) {
        ...
        rsvg_preserve_aspect_ratio(self->preserve_aspect_ratio, self->vbox.rect.width, self->vbox.rect.height, &w, &h,
                                   &x, &y);
    ...
    cairo_matrix_init_translate(&taffine, -_rsvg_css_normalize_length(&self->refX, ctx, 'h'),
    ...
    if (!state->overflow) {
        if (self->vbox.active)
            rsvg_add_clipping_rect(ctx, self->vbox.rect.x, self->vbox.rect.y, self->vbox.rect.width,
                                   self->vbox.rect.height);
        else
            rsvg_add_clipping_rect(ctx, 0, 0, _rsvg_css_normalize_length(&self->width, ctx, 'h'),
 ...
 *
 * 2. segment->is_degenerate = FALSE => either a lineto or a curveto (or the effective lineto that results from a
 * closepath). We have the following points: P1 = (p1x, p1y) P2 = (p2x, p2y) P3 = (p3x, p3y) P4 = (p4x, p4y)
 *
...
static void path_to_segments(const cairo_path_t* path, Segment** out_segments, int* num_segments) {
    int i;
...
static gboolean points_equal(double x1, double y1, double x2, double y2) {
 ...
 */
static gboolean is_zero_length_segment(Segment* segment) {
    double p1x, p1y;
    double p2x, p2y;
    double p3x, p3y;
    double p4x, p4y;
    ...
    return (points_equal(p1x, p1y, p2x, p2y) && points_equal(p1x, p1y, p3x, p3y) && points_equal(p1x, p1y, p4x, p4y));
}
 ...
 * in user space.
 */
static gboolean find_incoming_directionality_backwards(Segment* segments,
                                                       int num_segments,
                                                       int start_index,
                                                       double* vx,
                                                       double* vy) {
...
static gboolean find_outgoing_directionality_forwards(Segment* segments,
                                                      int num_segments,
                                                      int start_index,
                                                      double* vx,
                                                      double* vy) {
...
static double angle_from_vector(double vx, double vy) {
    double angle;
    ...
    for (i = 0; i < num_segments; i++) {
        ...
        if (segments[i].is_degenerate) {
            if (subpath_state == IN_SUBPATH) {
                ...
                rsvg_marker_render(endmarker, segments[i - 1].p4x, segments[i - 1].p4y,
            ...
            if (subpath_state == NO_SUBPATH) {
                ...
                rsvg_marker_render(startmarker, segments[i].p1x, segments[i].p1y,
                                   angle_from_vector(outgoing_vx, outgoing_vy), linewidth, ctx);
