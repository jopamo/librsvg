...
static GString* _rsvg_text_chomp(RsvgState* state, GString* in, gboolean* lastwasspace) {
    GString* out;
    guint i;
...
static void _rsvg_node_text_set_atts(RsvgNode* self, RsvgHandle* ctx, RsvgPropertyBag* atts) {
...
static void _rsvg_node_text_type_tspan(RsvgNodeText* self,
                                       RsvgDrawingCtx* ctx,
                                       gdouble* x,
                                       gdouble* y,
...
/* This function is responsible of selecting render for a text element including its children and giving it the drawing
 * context */
static void _rsvg_node_text_type_children(RsvgNode* self,
                                          RsvgDrawingCtx* ctx,
                                          gdouble* x,
                                          gdouble* y,
                                          gboolean* lastwasspace,
        ...
        if (type == RSVG_NODE_TYPE_CHARS) {
            ...
            if (usetextonly) {
                ...
                if (type == RSVG_NODE_TYPE_TSPAN) {
                    ...
                    rsvg_state_push(ctx);
                    _rsvg_node_text_type_tspan(tspan, ctx, x, y, lastwasspace, usetextonly);
                    rsvg_state_pop(ctx);
                }
            }
        }
...
static int _rsvg_node_text_length_tspan(RsvgNodeText* self,
                                        RsvgDrawingCtx* ctx,
                                        gdouble* x,
                                        gboolean* lastwasspace,
                                        gboolean usetextonly);
...
static int _rsvg_node_text_length_children(RsvgNode* self,
                                           RsvgDrawingCtx* ctx,
                                           gdouble* length,
                                           gboolean* lastwasspace,
                                           gboolean usetextonly) {
    ...
    for (i = 0; i < self->children->len; i++) {
        ...
        if (type == RSVG_NODE_TYPE_CHARS) {
            ...
            *length += rsvg_text_length_text_as_string(ctx, str->str);
            ...
            if (usetextonly) {
                out = _rsvg_node_text_length_children(node, ctx, length, lastwasspace, usetextonly);
            }
            else {
                if (type == RSVG_NODE_TYPE_TSPAN) {
                    RsvgNodeText* tspan = (RsvgNodeText*)node;
                    out = _rsvg_node_text_length_tspan(tspan, ctx, length, lastwasspace, usetextonly);
                }
            }
        }
        rsvg_state_pop(ctx);
...
static void _rsvg_node_text_draw(RsvgNode* self, RsvgDrawingCtx* ctx, int dominate) {
...
static void _rsvg_node_text_type_tspan(RsvgNodeText* self,
    ...
    if (rsvg_current_state(ctx)->text_anchor != TEXT_ANCHOR_START) {
        ...
        _rsvg_node_text_length_children(&self->super, ctx, &length, &lws, usetextonly);
        if (rsvg_current_state(ctx)->text_anchor == TEXT_ANCHOR_MIDDLE)
            length /= 2;
    }
    ...
    *y += dy;
    _rsvg_node_text_type_children(&self->super, ctx, x, y, lastwasspace, usetextonly);
}
...
static int _rsvg_node_text_length_tspan(RsvgNodeText* self,
    ...
    return _rsvg_node_text_length_children(&self->super, ctx, length, lastwasspace, usetextonly);
}

static void _rsvg_node_tspan_set_atts(RsvgNode* self, RsvgHandle* ctx, RsvgPropertyBag* atts) {
    const char *klazz = NULL, *id = NULL, *value;
    RsvgNodeText* text = (RsvgNodeText*)self;
...
static void rsvg_text_layout_free(RsvgTextLayout* layout) {
    g_object_unref(layout->layout);
    g_free(layout);
}

static PangoLayout* rsvg_text_create_layout(RsvgDrawingCtx* ctx,
                                            RsvgState* state,
                                            const char* text,
    ...
    attribute =
        pango_attr_letter_spacing_new(_rsvg_css_normalize_length(&state->letter_spacing, ctx, 'h') * PANGO_SCALE);
    ...
    pango_layout_set_alignment(layout, (state->text_dir == PANGO_DIRECTION_LTR) ? PANGO_ALIGN_LEFT : PANGO_ALIGN_RIGHT);

    return layout;
}

static RsvgTextLayout* rsvg_text_layout_new(RsvgDrawingCtx* ctx, RsvgState* state, const char* text) {
...
void rsvg_text_render_text(RsvgDrawingCtx* ctx, const char* text, gdouble* x, gdouble* y) {
    PangoContext* context;
    PangoLayout* layout;
...
static gdouble rsvg_text_layout_width(RsvgTextLayout* layout) {
    gint width;
